 ...
 * author: Alex
 *   date: 2018-03-05
 *   file: ZBRecvSendChatNoticeTask.cpp
...
ZBRecvSendChatNoticeTask::ZBRecvSendChatNoticeTask(void)
{
	m_listener = NULL;
...
// 初始化
bool ZBRecvSendChatNoticeTask::Init(IZBImClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool ZBRecvSendChatNoticeTask::Handle(const ZBTransportProtocol& tp)
{
	bool result = false;
	...
	// 通知listener
	if (NULL != m_listener) {
        m_listener->OnZBRecvSendChatNotice(m_roomId, m_level, m_fromId, m_nickName, m_Msg, m_HonorUrl);
...
// 获取待发送的Json数据
bool ZBRecvSendChatNoticeTask::GetSendData(Json::Value& data)
{
	bool result = false;
	
	FileLog("ImClient", "ZBRecvSendChatNoticeTask::GetSendData() begin");
    {
        // 构造json协议
        Json::Value value;
...
// 获取命令号
string ZBRecvSendChatNoticeTask::GetCmdCode() const
{
	return ZB_CMD_RECVSENDCHATNOTICE;
}

// 设置seq
void ZBRecvSendChatNoticeTask::SetSeq(SEQ_T seq)
{
	m_seq = seq;
}

// 获取seq
SEQ_T ZBRecvSendChatNoticeTask::GetSeq() const
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool ZBRecvSendChatNoticeTask::IsWaitToRespond() const
{
	return false;
}

// 获取处理结果
void ZBRecvSendChatNoticeTask::GetHandleResult(ZBLCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 未完成任务的断线通知
void ZBRecvSendChatNoticeTask::OnDisconnect()
{
