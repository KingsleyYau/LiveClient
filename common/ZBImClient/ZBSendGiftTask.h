 ...
 * author: Alex
 *   date: 2018-03-05
 *   file: ZBSendGiftTask.h
 *   desc: 5.1.发送直播间礼物消息（观众端发送直播间礼物消息，包括连击礼物）Task实现类
 */
...
class ZBSendGiftTask : public IZBTask
{
public:
	ZBSendGiftTask(void);
	virtual ~ZBSendGiftTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& roomId, const string nickName, const string giftId, const string giftName, bool isBackPack, int giftNum, bool multi_click, int multi_click_start, int multi_click_end, int multi_click_id);
...
private:
    ...
    bool            m_isBackPack; // 是否背包礼物（1:是 0:否）
