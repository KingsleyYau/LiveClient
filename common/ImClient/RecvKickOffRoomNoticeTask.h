 ...
 * author: Alex
 *   date: 2017-06-12
 *   file: RecvKickOffRoomNoticeTask.h
 *   desc: 3.10.接收观众踢出直播间通知（观众端／主播端接收观众踢出直播间通知）Task实现类
 */
...
class RecvKickOffRoomNoticeTask : public ITask
{
public:
	RecvKickOffRoomNoticeTask(void);
	virtual ~RecvKickOffRoomNoticeTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& roomId, const string& userId, const string& nickName);
