 ...
 * author: Alex
 *   date: 2017-06-12
 *   file: FansKickOffRoomTask.h
 *   desc: 3.9.主播踢观众出直播间（主播端把指定观众踢出直播间）Task实现类
 */
...
class FansKickOffRoomTask : public ITask
{
public:
	FansKickOffRoomTask(void);
	virtual ~FansKickOffRoomTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& roomId, const string userId);
