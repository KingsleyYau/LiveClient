 ...
 * author: Alex
 *   date: 2017-06-12
 *   file: SendRoomFavTask.h
 *   desc: 5.1.发送直播间点赞消息（观众端向直播间发送点赞消息）Task实现类
 */
...
class SendRoomFavTask : public ITask
{
public:
	SendRoomFavTask(void);
	virtual ~SendRoomFavTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& roomId, const string token, const string nickName);
