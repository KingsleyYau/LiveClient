 ...
 * author: Alex
 *   date: 2017-06-12
 *   file: RecvShutUpNoticeTask.h
 *   desc: 3.8.接收直播间禁言通知（观众端／主播端接收直播间禁言通知）Task实现类
 */
...
class RecvShutUpNoticeTask : public ITask
{
public:
	RecvShutUpNoticeTask(void);
	virtual ~RecvShutUpNoticeTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& roomId, const string& userId, const string& nickName, int timeOut);
...
private:
    ...
    string          m_userId;   // 被禁言用户ID
    string          m_nickName; // 被禁言用户昵称
    int             m_timeOut;  // 禁言时长（秒）
};
