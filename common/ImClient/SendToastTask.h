 ...
 * author: Alex
 *   date: 2017-08-15
 *   file: SendToastTask.h
 *   desc: 6.1.发送直播间弹幕消息（观众端发送直播间弹幕消息）Task实现类
 */
...
class SendToastTask : public ITask
{
public:
	SendToastTask(void);
	virtual ~SendToastTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& roomId, const string nickName, const string msg);
