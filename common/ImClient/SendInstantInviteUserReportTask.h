 ...
 * author: Alex
 *   date: 2017-12-07
 *   file: SendInstantInviteUserReportTask.h
 *   desc: 7.8.观众端是否显示主播立即私密邀请Task实现类
 */
...
class SendInstantInviteUserReportTask : public ITask
{
public:
	SendInstantInviteUserReportTask(void);
	virtual ~SendInstantInviteUserReportTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& inviteId, bool isShow);
...
private:
    ...
    bool            m_isShow;       // 观众端是否弹出邀请（整型）（0：否，1：是）
