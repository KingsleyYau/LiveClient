 ...
 * author: Alex
 *   date: 2017-06-12
 *   file: RecvRoomGiftNoticeTask.cpp
 *   desc: 6.2.接收直播间礼物通知（观众端／主播端接收直播间礼物消息，包括连击礼物）TaskTask实现类
 */
...
// 初始化
bool RecvRoomGiftNoticeTask::Init(IImClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool RecvRoomGiftNoticeTask::Handle(const TransportProtocol& tp)
{
	bool result = false;

	FileLog("LiveChatClient", "RecvRoomGiftNoticeTask::Handle() begin, tp.isRespond:%d, tp.cmd:%s, tp.reqId:%d"
	...
	FileLog("LiveChatClient", "RecvRoomGiftNoticeTask::Handle() m_errType:%d", m_errType);
	...
	FileLog("LiveChatClient", "RecvRoomGiftNoticeTask::Handle() end");

	return result;
}
	
// 获取待发送的Json数据
bool RecvRoomGiftNoticeTask::GetSendData(Json::Value& data)
{
	bool result = false;
	
	FileLog("LiveChatClient", "RecvRoomGiftNoticeTask::GetSendData() begin");
    {
        // 构造json协议
        Json::Value value;
	...
	FileLog("LiveChatClient", "RecvRoomGiftNoticeTask::GetSendData() end, result:%d", result);

	return result;
}

// 获取命令号
string RecvRoomGiftNoticeTask::GetCmdCode() const
{
	return CMD_RECVROOMGIFTNOTICE;
}

// 设置seq
void RecvRoomGiftNoticeTask::SetSeq(SEQ_T seq)
{
	m_seq = seq;
}

// 获取seq
SEQ_T RecvRoomGiftNoticeTask::GetSeq() const
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool RecvRoomGiftNoticeTask::IsWaitToRespond() const
{
	return false;
}

// 获取处理结果
void RecvRoomGiftNoticeTask::GetHandleResult(LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 初始化参数
bool RecvRoomGiftNoticeTask::InitParam(const string& roomId, const string fromId, const string nickName, const string giftId, int giftNum, bool multi_click, int multi_click_start, int multi_click_end)
{
	bool result = false;
    if (!roomId.empty()
        && !fromId.empty()
...
// 未完成任务的断线通知
void RecvRoomGiftNoticeTask::OnDisconnect()
{
}
