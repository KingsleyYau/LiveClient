...
void ImClient::OnConnect(bool success)
{
    ...
    if (success) {
        FileLog("ImClient", "ImClient::OnConnect()");
        // 开始登录
        LoginProc();
        // 启动发送心跳包线程
        HearbeatThreadStart();
...
bool ImClient::SendInstantInviteUserReport(SEQ_T reqId, const string& inviteId, bool isShow)
{
    ...
    FileLog("ImClient", "ImClient::SendInstantInviteUserReport() end, m_taskManager:%p inviteId:%s isShow:%d result:%d", m_taskManager,  inviteId.c_str(), isShow, result);
    return result;
}
...
{
    FileLog("ImClient", "ImClient::OnSendInstantInviteUserReport() begin, ImClient:%p reqId:%d err:%d errMsg:%s roomId:%s", this, reqId, err, errMsg.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
    ...
    FileLog("ImClient", "ImClient::OnSendInstantInviteUserReport() end, ImClient:%p reqId:%d err:%d errMsg:%s roomId:%s", this, reqId, err, errMsg.c_str());
 ...
 *
 */
void ImClient::OnSendTalent(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg, const string& talentInviteId, const string& talentId)
{
    FileLog("ImClient", "ImClient::OnSendTalent() begin, ImClient:%p reqId:%d err:%d errMsg:%s talentInviteId:%s talentId:%s", this, reqId, err, errMsg.c_str(), talentInviteId.c_str(), talentId.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnSendTalent(reqId, success,err, errMsg, talentInviteId, talentId);
    }
    FileLog("ImClient", "ImClient::OnSendTalent() end, ImClient:%p reqId:%d err:%d errMsg:%s talentInviteId:%s", this, reqId, err, errMsg.c_str(), talentInviteId.c_str());
...
bool ImClient::EnterHangoutRoom(SEQ_T reqId, const string& roomId) {
    ...
    FileLog("ImClient", "ImClient::EnterHangoutRoom() end, m_taskManager:%p roomId:%s result:%d", m_taskManager, roomId.c_str(), result);
    return result;
}
 ...
 *
 */
void ImClient::OnEnterHangoutRoom(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg, const IMHangoutRoomItem& item) {
    FileLog("ImClient", "ImClient::OnEnterHangoutRoom() begin, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnEnterHangoutRoom(reqId, success,err, errMsg, item);
    }
    FileLog("ImClient", "ImClient::OnEnterHangoutRoom() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
 ...
 *
 */
bool ImClient::LeaveHangoutRoom(SEQ_T reqId, const string& roomId) {
    bool result = false;
    m_loginStatusLock->Lock();
    LoginStatus loginStatus = m_loginStatus;
    m_loginStatusLock->Unlock();
    FileLog("ImClient", "ImClient::LeaveHangoutRoom() begin, m_taskManager:%p roomId:%s", m_taskManager, roomId.c_str());
    // 若为已登录状态
    if (LOGINED == loginStatus) {
        LeaveHangoutRoomTask* task = new LeaveHangoutRoomTask();
        if (NULL != task) {
            task->Init(this);
            task->InitParam(roomId);
    ...
    FileLog("ImClient", "ImClient::LeaveHangoutRoom() end, m_taskManager:%p roomId:%s result:%d", m_taskManager, roomId.c_str(), result);
    return result;
}
 ...
 *
 */
void ImClient::OnLeaveHangoutRoom(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg) {
    FileLog("ImClient", "ImClient::OnLeaveHangoutRoom() begin, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnLeaveHangoutRoom(reqId, success,err, errMsg);
    }
    FileLog("ImClient", "ImClient::OnLeaveHangoutRoom() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
 ...
 *
 */
bool ImClient::SendHangoutGift(SEQ_T reqId, const string& roomId, const string& nickName, const string& toUid, const string& giftId, const string& giftName, bool isBackPack, int giftNum, bool isMultiClick, int multiClickStart, int multiClickEnd, int multiClickId, bool isPrivate)  {
    bool result = false;
    m_loginStatusLock->Lock();
    LoginStatus loginStatus = m_loginStatus;
    m_loginStatusLock->Unlock();
    FileLog("ImClient", "ImClient::SendHangoutGift() begin, m_taskManager:%p roomId:%s", m_taskManager, roomId.c_str());
    // 若为已登录状态
    if (LOGINED == loginStatus) {
        SendHangoutGiftTask* task = new SendHangoutGiftTask();
        if (NULL != task) {
            task->Init(this);
    ...
    FileLog("ImClient", "ImClient::SendHangoutGift() end, m_taskManager:%p roomId:%s result:%d", m_taskManager, roomId.c_str(), result);
    return result;
}
 ...
 *
 */
void ImClient::OnSendHangoutGift(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg, double credit) {
    FileLog("ImClient", "ImClient::OnSendHangoutGift() begin, ImClient:%p reqId:%d err:%d errMsg:%s credit:%f", this, reqId, err, errMsg.c_str(), credit);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnSendHangoutGift(reqId, success,err, errMsg, credit);
    }
    FileLog("ImClient", "ImClient::OnSendHangoutGift() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
 ...
 *
 */
bool ImClient::ControlManPushHangout(SEQ_T reqId, const string& roomId, IMControlType control) {
    bool result = false;
    m_loginStatusLock->Lock();
    LoginStatus loginStatus = m_loginStatus;
    m_loginStatusLock->Unlock();
    FileLog("ImClient", "ImClient::ControlManPushHangout() begin, m_taskManager:%p roomId:%s control;%d", m_taskManager, roomId.c_str(), control);
    // 若为已登录状态
    if (LOGINED == loginStatus) {
        ControlManPushHangoutTask* task = new ControlManPushHangoutTask();
        if (NULL != task) {
            task->Init(this);
            task->InitParam(roomId, control);
    ...
    FileLog("ImClient", "ImClient::ControlManPushHangout() end, m_taskManager:%p roomId:%s control;%d result:%d", m_taskManager, roomId.c_str(), control, result);
    return result;
}
 ...
 *
 */
void ImClient::OnControlManPushHangout(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg, const list<string>& manPushUrl) {
    FileLog("ImClient", "ImClient::OnControlManPushHangout() begin, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnControlManPushHangout(reqId, success,err, errMsg, manPushUrl);
    }
    FileLog("ImClient", "ImClient::OnControlManPushHangout() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
 ...
 *
 */
bool ImClient::SendHangoutLiveChat(SEQ_T reqId, const string& roomId, const string& nickName, const string& msg, const list<string>& at) {
    bool result = false;
    m_loginStatusLock->Lock();
    LoginStatus loginStatus = m_loginStatus;
    m_loginStatusLock->Unlock();
    FileLog("ImClient", "ImClient::SendHangoutLiveChat() begin, m_taskManager:%p roomId:%s nickName;%s msg:%s", m_taskManager, roomId.c_str(), nickName.c_str(), msg.c_str());
    // 若为已登录状态
    if (LOGINED == loginStatus) {
        SendHangoutLiveChatTask* task = new SendHangoutLiveChatTask();
        if (NULL != task) {
            task->Init(this);
            task->InitParam(roomId, nickName, msg, at);
    ...
    FileLog("ImClient", "ImClient::SendHangoutLiveChat() end, m_taskManager:%p roomId:%s result:%d", m_taskManager, roomId.c_str(), result);
    return result;
}
/**
 ...
 *
 */
void ImClient::OnSendHangoutLiveChat(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg) {
    FileLog("ImClient", "ImClient::OnSendHangoutLiveChat() begin, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnSendHangoutLiveChat(reqId, success,err, errMsg);
    }
    FileLog("ImClient", "ImClient::OnSendHangoutLiveChat() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
    
    m_listenerListLock->Unlock();
}

bool ImClient::SendPrivateMessage(SEQ_T reqId, const string& toId, const string& content, int sendMsgId) {
    bool result = false;
    m_loginStatusLock->Lock();
    LoginStatus loginStatus = m_loginStatus;
    m_loginStatusLock->Unlock();
    FileLog("ImClient", "ImClient::SendPrivateMessage() begin, m_taskManager:%p toId:%s content:%s sendMsgId:%d", m_taskManager, toId.c_str(), content.c_str(), sendMsgId);
    // 若为已登录状态
    if (LOGINED == loginStatus) {
        SendPrivateMessageTask* task = new SendPrivateMessageTask();
        if (NULL != task) {
            task->Init(this);
            task->InitParam(toId, content, sendMsgId);
    ...
    FileLog("ImClient", "ImClient::SendPrivateMessage() end, m_taskManager:%p toId:%s result:%d", m_taskManager, toId.c_str(), result);
    return result;
}

void ImClient::OnSendPrivateMessage(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg, int messageId, double credit, const string& toId, int sendMsgId) {
    FileLog("ImClient", "ImClient::OnSendPrivateMessage() begin, ImClient:%p reqId:%d err:%d errMsg:%s messageId:%d credit;%f toId:%s sendMsgId:%d", this, reqId, err, errMsg.c_str(), messageId, credit, toId.c_str(), sendMsgId);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end(); itr++) {
        IImClientListener* callback = *itr;
        callback->OnSendPrivateMessage(reqId, success,err, errMsg, messageId, credit, toId, sendMsgId);
    }
    FileLog("ImClient", "ImClient::OnSendPrivateMessage() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, err, errMsg.c_str());
...
void ImClient::OnRecvRoomCloseNotice(const string& roomId, LCC_ERR_TYPE err, const string& errMsg, const IMAuthorityItem& priv) {
    FileLog("ImClient", "ImClient::OnRecvRoomCloseNotice() begin, ImClient:%p roomId:%s", this, roomId.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
    ...
    FileLog("ImClient", "ImClient::OnRecvRoomCloseNotice() end, ImClient:%p roomId:%s", this, roomId.c_str());
    m_listenerListLock->Unlock();
    
}

// 3.4.接收观众进入直播间通知
void ImClient::OnRecvEnterRoomNotice(const string& roomId, const string& userId, const string& nickName, const string& photoUrl, const string& riderId, const string& riderName, const string& riderUrl, int fansNum, const string& honorImg, bool isHasTicket) {
    FileLog("ImClient", "ImClient::OnRecvEnterRoomNotice() begin, ImClient:%p roomId:%s userId:%s nickName:%s photoUrl:%s riderId;%s riderName:%s riderUrl:%s fansNum:%d honorImg:%s isHasTicket:%d", this, roomId.c_str(), userId.c_str(), nickName.c_str(), photoUrl.c_str(), riderId.c_str(), riderName.c_str(), riderUrl.c_str(), fansNum, honorImg.c_str(), isHasTicket);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvEnterRoomNotice(roomId, userId, nickName, photoUrl, riderId, riderName, riderUrl, fansNum, honorImg, isHasTicket);
    }
    FileLog("ImClient", "ImClient::OnRecvEnterRoomNotice() end, ImClient:%p roomId:%s userId:%s nickName:%s photoUrl:%s riderId:%s riderName:%s riderUrl:%s fansNum:%d honorImg:%s isHasTicket:%d", this, roomId.c_str(), userId.c_str(), nickName.c_str(), photoUrl.c_str(), riderId.c_str(), riderName.c_str(), riderUrl.c_str(), fansNum, honorImg.c_str(), isHasTicket);
    m_listenerListLock->Unlock();
}
...
void ImClient::OnRecvRoomKickoffNotice(const string& roomId, LCC_ERR_TYPE err, const string& errMsg, double credit, const IMAuthorityItem& item) {
   ...
   FileLog("ImClient", "ImClient::OnRecvRoomKickoffNotice() end, ImClient:%p roomId:%s err:%d errMsg:%s credit:%f", this, roomId.c_str(), err, errMsg.c_str(), credit);
    m_listenerListLock->Unlock();
}
...
void ImClient::OnRecvCreditNotice(const string& roomId, double credit) {
    ...
    FileLog("ImClient", "ImClient::OnRecvCreditNotice() end, ImClient:%p roomId:%s credit:%f", this, roomId.c_str(), credit);
    m_listenerListLock->Unlock();
...
void ImClient::OnRecvWaitStartOverNotice(const StartOverRoomItem& item)
{
    ...
    FileLog("ImClient", "ImClient::OnRecvWaitStartOverNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvChangeVideoUrl(const string& roomId, bool isAnchor, const list<string>& playUrl, const string& userId)
{
    FileLog("ImClient", "ImClient::OnRecvChangeVideoUrl() begin, ImClient:%p roomId:%s isAnchor:%d userId:%s", this, roomId.c_str(), isAnchor, userId.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvChangeVideoUrl(roomId, isAnchor, playUrl, userId);
    }
    FileLog("ImClient", "ImClient::OnRecvChangeVideoUrl() end, ImClient:%p roomId:%s isAnchor:%d userId:%s", this, roomId.c_str(), isAnchor, userId.c_str());
    m_listenerListLock->Unlock();
}
...
void ImClient::OnRecvTalentListNotice(const string& roomId, const string& introduction)
{
    FileLog("ImClient", "ImClient::OnRecvTalentListNotice() begin, ImClient:%p roomId:%s introduction:%s", this, roomId.c_str(), introduction.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvTalentListNotice(roomId, introduction);
    }
    FileLog("ImClient", "ImClient::OnRecvTalentListNotice() end,  ImClient:%p roomId:%s introduction:%s", this, roomId.c_str(), introduction.c_str());
    m_listenerListLock->Unlock();
}
...
void ImClient::OnRecvLoveLevelUpNotice(const IMLoveLevelItem& loveLevelItem)
{
    FileLog("ImClient", "ImClient::OnRecvLoveLevelUpNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvLoveLevelUpNotice(loveLevelItem);
    }
    FileLog("ImClient", "ImClient::OnRecvLoveLevelUpNotice() end,  ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvRecommendHangoutNotice(const IMRecommendHangoutItem& item) {
    FileLog("ImClient", "ImClient::OnRecvRecommendHangoutNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvRecommendHangoutNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvRecommendHangoutNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvDealInviteHangoutNotice(const IMRecvDealInviteItem& item) {
    FileLog("ImClient", "ImClient::OnRecvDealInviteHangoutNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvDealInviteHangoutNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvDealInviteHangoutNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvEnterHangoutRoomNotice(const IMRecvEnterRoomItem& item) {
    FileLog("ImClient", "ImClient::OnRecvEnterHangoutRoomNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvEnterHangoutRoomNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvEnterHangoutRoomNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvLeaveHangoutRoomNotice(const IMRecvLeaveRoomItem& item) {
    FileLog("ImClient", "ImClient::OnRecvLeaveHangoutRoomNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvLeaveHangoutRoomNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvLeaveHangoutRoomNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvHangoutGiftNotice(const IMRecvHangoutGiftItem& item) {
    FileLog("ImClient", "ImClient::OnRecvHangoutGiftNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvHangoutGiftNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvHangoutGiftNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvKnockRequestNotice(const IMKnockRequestItem& item) {
    FileLog("ImClient", "ImClient::OnRecvKnockRequestNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvKnockRequestNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvKnockRequestNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvLackCreditHangoutNotice(const IMLackCreditHangoutItem& item) {
    FileLog("ImClient", "ImClient::OnRecvLackCreditHangoutNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvLackCreditHangoutNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvLackCreditHangoutNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvHangoutChatNotice(const IMRecvHangoutChatItem& item) {
    FileLog("ImClient", "ImClient::OnRecvHangoutChatNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvHangoutChatNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvHangoutChatNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}

void ImClient::OnRecvAnchorCountDownEnterHangoutRoomNotice(const string& roomId, const string& anchorId, int leftSecond) {
    FileLog("ImClient", "ImClient::OnRecvAnchorCountDownEnterHangoutRoomNotice() begin, ImClient:%p roomId:%s anchorId:%s leftSecond:%d", this, roomId.c_str(), anchorId.c_str(), leftSecond);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
 ...
 *
 */
void ImClient::OnRecvProgramPlayNotice(const IMProgramItem& item, IMProgramNoticeType type, const string& msg) {
    FileLog("ImClient", "ImClient::OnRecvProgramPlayNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvProgramPlayNotice(item, type, msg);
    }
    FileLog("ImClient", "ImClient::OnRecvProgramPlayNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvCancelProgramNotice(const IMProgramItem& item) {
    FileLog("ImClient", "ImClient::OnRecvCancelProgramNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvCancelProgramNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvCancelProgramNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
 ...
 *
 */
void ImClient::OnRecvRetTicketNotice(const IMProgramItem& item, double leftCredit) {
    FileLog("ImClient", "ImClient::OnRecvRetTicketNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvRetTicketNotice(item, leftCredit);
    }
    FileLog("ImClient", "ImClient::OnRecvRetTicketNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}

void ImClient::OnRecvPrivateMessageNotice(const IMPrivateMessageItem& item) {
    FileLog("ImClient", "ImClient::OnRecvPrivateMessageNotice() begin, ImClient:%p", this);
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvPrivateMessageNotice(item);
    }
    FileLog("ImClient", "ImClient::OnRecvPrivateMessageNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}

void ImClient::OnRecvLoiNotice(const string& anchorId, const string& loiId) {
    FileLog("ImClient", "ImClient::OnRecvLoiNotice() begin, ImClient:%p anchorId:%s loiId:%s", this, anchorId.c_str(), loiId.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvLoiNotice(anchorId, loiId);
    }
    FileLog("ImClient", "ImClient::OnRecvLoiNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}

void ImClient::OnRecvEMFNotice(const string& anchorId, const string& emfId) {
    FileLog("ImClient", "ImClient::OnRecvEMFNotice() begin, ImClient:%p anchorId:%s emfId:%s", this, anchorId.c_str(), emfId.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
         itr != m_listenerList.end();
         itr++) {
        IImClientListener* callback = *itr;
        callback->OnRecvEMFNotice(anchorId, emfId);
    }
    FileLog("ImClient", "ImClient::OnRecvEMFNotice() end, ImClient:%p", this);
    m_listenerListLock->Unlock();
}
