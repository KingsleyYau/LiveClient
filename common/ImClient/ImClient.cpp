...
// 登录
bool ImClient::Login(const string& user, const string& password)
{
	bool result = false;
...
bool ImClient::LoginProc()
{
    ...
    if (NULL != task) {
        task->Init(this);
        task->InitParam(IMCLIENT_VERSION, m_user, m_password);
...
void ImClient::OnFansRoomOut(SEQ_T reqId, bool success, LCC_ERR_TYPE err, const string& errMsg) {
    FileLog("ImClient", "ImClient::OnFansRoomOut() begin, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, success, err, errMsg.c_str());
    m_listenerListLock->Lock();
    for (ImClientListenerList::const_iterator itr = m_listenerList.begin();
    ...
    FileLog("ImClient", "ImClient::OnFansRoomOut() end, ImClient:%p reqId:%d err:%d errMsg:%s", this, reqId, success, err, errMsg.c_str());
    m_listenerListLock->Unlock();
}
// 获取直播间信息
bool ImClient::GetRoomInfo(SEQ_T reqId, const string& token, const string& roomId) {
    ...
    if (LOGINED == loginStatus) {
        ...
        if (NULL != task) {
            ...
            SEQ_T seq = m_seqCounter.GetAndDecrement();
            task->SetSeq(seq);
            result = m_taskManager->HandleRequestTask(task);
        }
    }
