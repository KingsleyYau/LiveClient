package com.qpidnetwork.anchor.liveshow.liveroom.barrage;
...
import com.qpidnetwork.anchor.liveshow.liveroom.BaseAnchorLiveRoomActivity;
...
public class BarrageManager<T> {
    private final String TAG = BarrageManager.class.getSimpleName();
    private WeakReference<BaseAnchorLiveRoomActivity> mActivity;
    ...
    private boolean isBarrageViewOutAnimPlaying = false;


    public BarrageManager(BaseAnchorLiveRoomActivity context, List<View> listViews, View ll_bulletScreen){
        this.mActivity = new WeakReference<BaseAnchorLiveRoomActivity>(context);
            ...
            if(null != mActivity && null != mActivity.get()){
                ll_bulletScreen.setBackgroundDrawable(isNeedShowBg ?
                        mActivity.get().roomThemeManager.getRoomBarrageBgDrawable(
                                mActivity.get().getCurrentRoomType()): new ColorDrawable(Color.TRANSPARENT));
            }
        }
    }
    ...
    private synchronized void startAnimation(View view, T barrageItem){
        ...
        animationSet.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                View view = null;
                synchronized (mAnimationViewMap){
                    view = mAnimationViewMap.remove(animation);
                }
                if(view != null) {
    ...
    private void playBarrageViewEnterAnim(){
        if(View.INVISIBLE == ll_bulletScreen.getVisibility()){
            ...
            ll_bulletScreen.clearAnimation();
        }
    }

    private void playBarrageViewOutAnim(){
        if(View.VISIBLE == ll_bulletScreen.getVisibility()){
            isBarrageViewOutAnimPlaying = true;
            final AnimationSet animationSet1 = (AnimationSet) AnimationUtils.loadAnimation(mActivity.get(), R.anim.barrage_view_exit);
            animationSet1.setAnimationListener(new Animation.AnimationListener() {
                @Override
                public void onAnimationStart(Animation animation) {
