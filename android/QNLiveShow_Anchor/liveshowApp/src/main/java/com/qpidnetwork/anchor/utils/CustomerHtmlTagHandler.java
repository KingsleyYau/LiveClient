...
public class CustomerHtmlTagHandler implements Html.TagHandler {
    ...
    public static class Builder {
        ...
        private int riderImgWidth;
        private int riderImgHeight;
        ...
        public Builder setRiderImgWidth(int riderImgWidth){
            this.riderImgWidth = riderImgWidth;
            return this;
        }

        public int getRiderImgWidth(){
            return riderImgWidth;
        }

        public Builder setRiderImgHeight(int riderImgHeight){
            this.riderImgHeight = riderImgHeight;
            return this;
        }

        public int getRiderImgHeight(){
            return riderImgHeight;
        }
...
//        }
    ...
    public Drawable getDrawable(String source) {
        ...
        try {
            switch (imageType){
                ...
                case Rider:
                    String riderLocalPath = source.substring(5, source.length());
                    Log.d(TAG,"getDrawable-rider-riderLocalPath:"+riderLocalPath);
                    drawable = getRiderDrawable(riderLocalPath);
                    break;
                case AnchorA:
                    drawable = this.mBuilder.getContext().getResources().getDrawable(R.drawable.ic_live_room_msgitem_anchor_flag);
    ...
    private Drawable getRiderDrawable(String carImgLocalPath){
        boolean fileExists = SystemUtils.fileExists(carImgLocalPath);
        Drawable drawable = null;
        if(fileExists){
            drawable = new BitmapDrawable(ImageUtil.decodeSampledBitmapFromFile(carImgLocalPath,
                    this.mBuilder.getRiderImgWidth(),
                    this.mBuilder.getRiderImgHeight()));
            drawable.setBounds(0, 0, this.mBuilder.getRiderImgWidth(),this.mBuilder.getRiderImgHeight());
        }
        return drawable;
    }
    ...
    private HtmlImageType getImageType(String source){
        ...
        }else if (source.startsWith("rider")){
            imageType = HtmlImageType.Rider;
