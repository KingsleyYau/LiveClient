package com.qpidnetwork.anchor.framework.widget.picasso;
...
public final class OkHttp3Downloader implements Downloader {
  private static final String TAG = OkHttp3Downloader.class.getSimpleName();
  ...
  @Override public Response load(Uri uri, int networkPolicy) throws IOException {
    Log.d(TAG,"load-uri:"+uri+" networkPolicy:"+networkPolicy);
    CacheControl cacheControl = null;
    if (networkPolicy != 0) {
      boolean isOfflineOnly = NetworkPolicy.isOfflineOnly(networkPolicy);
      Log.d(TAG,"load-isOfflineOnly:"+isOfflineOnly);
      if (isOfflineOnly) {
        cacheControl = CacheControl.FORCE_CACHE;
      } else {
        CacheControl.Builder builder = new CacheControl.Builder();
        boolean shouldReadFromDiskCache = NetworkPolicy.shouldReadFromDiskCache(networkPolicy);
        Log.d(TAG,"load-shouldReadFromDiskCache:"+shouldReadFromDiskCache);
        if (!shouldReadFromDiskCache) {
          builder.noCache();
        }
        boolean shouldWriteToDiskCache = NetworkPolicy.shouldWriteToDiskCache(networkPolicy);
        Log.d(TAG,"load-shouldWriteToDiskCache:"+shouldWriteToDiskCache);
        if (!shouldWriteToDiskCache) {
          builder.noStore();
        }
    ...
    Log.d(TAG,"load-responseCode:"+responseCode+" msg:"+response.message());
    if (responseCode >= 300) {
      response.body().close();
