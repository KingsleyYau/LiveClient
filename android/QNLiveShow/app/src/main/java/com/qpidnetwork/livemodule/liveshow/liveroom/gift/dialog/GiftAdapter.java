...
public class GiftAdapter <T> extends CanAdapter {
    ...
    protected void setView(CanHolderHelper helper, int position, Object bean) {
        ...
        if(isSelected && item.giftType == GiftItem.GiftType.Advanced && !TextUtils.isEmpty(item.srcWebpUrl)){
            String localPath = FileCacheManager.getInstance().getGiftLocalPath(item.id, item.srcWebpUrl);
            pb_loadAdvanceAnimFile.setVisibility(SystemUtils.fileExists(localPath) ? View.GONE : View.VISIBLE);
        }else{
        ...
        tv_giftCoins.setText(LiveApplication.getContext().getResources().getString(R.string.live_gift_coins,String.valueOf(item.credit)));
