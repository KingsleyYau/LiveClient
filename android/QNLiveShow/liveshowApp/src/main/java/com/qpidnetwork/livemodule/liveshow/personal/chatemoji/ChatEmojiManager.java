...
public class ChatEmojiManager {
    ...
    private Map<String,String> emotionIdUrlMaps = new HashMap<>();
...
//------------------------------构造&set、get方法---------------------
    ...
    public void getEmojiList(final OnGetEmotionListCallback callback){
        ...
        if(HttpReqStatus.Reqing != emojiListReqStatus){
            ...
            RequestJniLiveShow.GetEmotionList(new OnGetEmotionListCallback() {
                @Override
    ...
    private void downEmotionImg(EmotionItem emotionItem){
        Log.d(TAG,"downEmotionImg-emotionItem.id:"+emotionItem.emotionId
                +" emotionItem.emoSign:"+emotionItem.emoSign
                +" emotionItem.emoUrl:"+emotionItem.emoUrl);
        if(!TextUtils.isEmpty(emotionItem.emoUrl)){
            //图片下载
            ...
            if(!SystemUtils.fileExists(localPath)){
                Log.d(TAG,"downEmotionImg-本地图片不存在，开始下载");
                FileDownloadManager.getInstance().start(emotionItem.emoUrl, localPath, new IFileDownloadedListener(){
                    public void onCompleted(boolean isSuccess, String localFilePath, String fileUrl){
                        Log.d(TAG,"downEmotionImg-onCompleted-isSuccess:"+isSuccess
...
//------------------------------用于TextView的显示---------------------
    ...
    public Spanned parseEmoji(final Context context, final String str, int model, final int width, final int height){
        Log.d(TAG,"parseEmoji-str:"+str+" model:"+model+" width:"+width+" height:"+height);
    ...
    private String parseEmojiToImage(String str, Pattern patten,
        ...
        Log.d(TAG,"parseEmojiToImage-str:"+str+" start:"+start);
