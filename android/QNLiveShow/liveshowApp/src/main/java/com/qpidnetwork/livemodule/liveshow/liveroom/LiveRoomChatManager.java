...
public class LiveRoomChatManager implements IFileDownloadedListener {
    ...
    private void refreshViewByMessageItem(ViewHolder holder,
        ...
        switch (liveMsgItem.msgType){
            ...
            case Barrage:{
                //弹幕或者普通文本消息
                span = ChatEmojiManager.getInstance().parseEmoji(mActivity.get(),
                        mContext.getResources().getString(
                                mActivity.get().roomThemeManager.getRoomMsgListTxtMsgStrResId(
                                        mActivity.get().mIMRoomInItem.roomType),
                                liveMsgItem.nickName,
                                liveMsgItem.textMsgContent.message),
                        ChatEmojiManager.PATTERN_MODEL_EVERYSIGN,imgWidth,imgHeight);
            }break;
            case Gift:{
                ...
                span = mImageGetter.getExpressMsgHTML(mContext.getResources().getString(
                        mActivity.get().roomThemeManager.getRoomMsgListGiftMsgStrResId(mActivity.get().mIMRoomInItem.roomType),
                        liveMsgItem.nickName,
                        liveMsgItem.giftMsgContent.giftName,
                        liveMsgItem.giftMsgContent.giftId, giftNum));
            }break;
            case FollowHost:{//TODO;ask lvy for msg type aval?
            ...
            case RoomIn:{
                span = Html.fromHtml(mContext.getResources().getString(
                        mActivity.get().roomThemeManager.getRoomMsgListRoomInMsgStrResId(mActivity.get().mIMRoomInItem.roomType),
                        liveMsgItem.nickName));
            }break;
