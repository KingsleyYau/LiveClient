 ...
 * 直播间聊天消息列表item文本处理
 */
public class LiveRoomChatMsgListItemTextManager {
    /**
     * html模板中, 已购票标识
     */
    ...
    public LiveRoomChatMsgListItemTextManager(Context context ,
        ...
        switch (msgListItem.imMessageItem.msgType){
            ...
            case Barrage:{
                ...
                if(0 != strResId){
                    //把数据填入HTML模块中
                    String htmlStr = mContext.getResources().getString(strResId,
                            msgListItem.imMessageItem.nickName,                     // 2019/9/11 Hardy 昵称占位字符串
                            TextUtils.htmlEncode(msgListItem.imMessageItem.textMsgContent.message));
...
//                });
            ...
            case Voucher:
                final IMVoucherMessageContent msgContent = msgListItem.imMessageItem.voucherMessageContent;
                if (msgContent != null) {
                    int strResId = mRoomThemeManager.getFullScreenRoomVoucherMsgStrResId(currLiveRoomType);
                    if(0 != strResId) {

                        //把数据填入HTML模块中
                        String htmlStr = mContext.getResources().getString(
                                strResId, msgContent.useCoupon);
                        msgListItem.spanned = HtmlSpannedHandler.getLiveRoomMsgHTML(mBuilder , htmlStr,false);
                    }
                }
                break;
            default:
