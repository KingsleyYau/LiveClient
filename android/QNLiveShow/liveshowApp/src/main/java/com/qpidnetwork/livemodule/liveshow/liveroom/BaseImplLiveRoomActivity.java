...
public class BaseImplLiveRoomActivity extends BaseFragmentActivity
    ...
    public void OnRecvRebateInfoNotice(String roomId, IMRebateItem item) {
        ...
        if(isCurrentRoom(roomId)) {
            if(mLiveRoomCreditRebateManager.getImRebateItem().cur_credit<item.cur_credit){
                addRebateGrantedMsg(item.cur_credit-mLiveRoomCreditRebateManager.getImRebateItem().cur_credit);
            }
...
//        loginItem = LoginManager.getInstance().getLoginItem();
    ...
    public void onGiftRecommand(GiftItem giftItem) {
        Log.d(TAG,"onGiftRecommand-giftItem.id:"+giftItem.id+" giftItem.name:"+giftItem.name);
    }
