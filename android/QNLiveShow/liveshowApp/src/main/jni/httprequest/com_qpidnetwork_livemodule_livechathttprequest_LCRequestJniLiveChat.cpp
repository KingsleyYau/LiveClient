...
void RequestLiveChatControllerCallback::OnUploadVoice(long requestId, bool success, const string& errnum, const string& errmsg, const string& voiceId)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::onUploadVoice( success : %s, env:%p, isAttachThread:%d )", success?"true":"false", env, isAttachThread);
...
{
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::UploadVoice() begin");
	...
	if (requestId != -1) {
		...
		FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::UploadVoice() fails. "
			"requestId:%lld"
			", voiceCode:%s"
			", inviteId:%s"
			", mineId:%s"
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::UploadVoice() requestId:%lld", requestId);
  ...
  (JNIEnv *env, jclass cls, jstring voiceId, jint siteType, jstring filePath, jobject callback)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::PlayVoice(). "
	...
	if (requestId != -1) {
		...
		FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::PlayVoice() fails. "
			"requestId:%lld, voiceId:%s, siteType:%d, filePath:%s"
			, requestId, strVoiceId.c_str(), siteType, strFilePath.c_str());
	}
	return requestId;
...
void RequestLiveChatControllerCallback::OnSendPhoto(long requestId, bool success, const string& errnum, const string& errmsg, const LSLCLCSendPhotoItem& item)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::onSendPhoto( success : %s, env:%p, isAttachThread:%d )", success?"true":"false", env, isAttachThread);
  ...
  (JNIEnv *env, jclass cls, jstring targetId, jstring inviteId, jstring userId, jstring sid, jstring filePath, jobject callback)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::SendPhoto() "
...
void RequestLiveChatControllerCallback::OnPhotoFee(long requestId, bool success, const string& errnum, const string& errmsg, const string& sendId)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::onPhotoFee( success : %s, env:%p, isAttachThread:%d )", success?"true":"false", env, isAttachThread);
  ...
  (JNIEnv *env, jclass cls, jstring targetId, jstring inviteId, jstring userId, jstring sid, jstring photoId, jobject callback)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::PhotoFee()"
	...
	if (requestId != -1) {
		...
		FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::PhotoFee() fails. "
			"requestId:%lld, targetId:%s, inviteId:%s, userId:%s, sid:%s, photoId:%s"
			, requestId, strTargetId.c_str(), strInviteId.c_str(), strUserId.c_str(), strSid.c_str(), strPhotoId.c_str());
	}
...
/********************************** 12.11.检查私密照片是否已付费 ********************************************/
void RequestLiveChatControllerCallback::OnCheckPhoto(long requestId, bool success, const string& errnum, const string& errmsg, const string& sendId, bool isCharge)
{
	JNIEnv* env = NULL;
	bool isAttachThread = false;
	GetEnv(&env, &isAttachThread);

	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::OnCheckPhoto( success : %s, env:%p, isAttachThread:%d )", success?"true":"false", env, isAttachThread);
	...
	string signure = "(JZLjava/lang/String;Ljava/lang/String;Ljava/lang/String;Z)V";
	jmethodID jCallback = env->GetMethodID(jCallbackCls, "OnCheckPhoto", signure.c_str());
	...
	if( jCallbackObj != NULL && jCallback != NULL ) {
		...
		env->CallVoidMethod(jCallbackObj, jCallback, jlrequestId, success, jerrno, jerrmsg, jsendId, isCharge);
 ...
 * Class:     com_qpidnetwork_livemodule_livechathttprequest_LCRequestJniLiveChat
 * Method:    PhotoFee
 * Signature: (Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Lcom/qpidnetwork/request/OnLCCheckPhotoCallback;)J
 */
JNIEXPORT jlong JNICALL Java_com_qpidnetwork_livemodule_livechathttprequest_LCRequestJniLiveChat_CheckPhoto
  (JNIEnv *env, jclass cls, jstring targetId, jstring inviteId, jstring userId, jstring sid, jstring photoId, jobject callback)
{
	jlong requestId = -1;
	const char *cpTemp = NULL;
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::CheckPhoto()"
    		"requestId:%lld, targetId:%s, inviteId:%s, userId:%s, sid:%s, photoId:%s"
    		, requestId, strTargetId.c_str(), strInviteId.c_str(), strUserId.c_str(), strSid.c_str(), strPhotoId.c_str());

	requestId = gLSLiveChatRequestLiveChatController.CheckPhoto(strTargetId, strInviteId, strUserId, strSid, strPhotoId);
	if (requestId != -1) {
		jobject obj = env->NewGlobalRef(callback);
		gCallbackMap.Insert(requestId, obj);
	}
	else {
		FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::CheckPhoto() fails. "
			"requestId:%lld, targetId:%s, inviteId:%s, userId:%s, sid:%s, photoId:%s"
			, requestId, strTargetId.c_str(), strInviteId.c_str(), strUserId.c_str(), strSid.c_str(), strPhotoId.c_str());
	}
  ...
  (JNIEnv *env, jclass cls, int toFlag, jstring targetId, jstring userId, jstring sid, jstring photoId, int size, int mode, jstring filePath, jobject callback)
{
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::GetPhoto() "
	...
	if (requestId != -1) {
		...
		FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::GetPhoto() fails. "
			"requestId:%lld, toFlag:%d, targetId:%s, userId:%s, sid:%s, photoId:%s, size:%d, mode:%d, filePath:%s"
			, requestId, toFlag, strTargetId.c_str(), strUserId.c_str(), strSid.c_str(), strPhotoId.c_str(), size, mode, strFilePath.c_str());
	}
	return requestId;
	...
	if( itr != gJavaItemMap.end() ) {
		...
		if( itemList.size() > 0 ) {
			...
			for(list<LSLCVideoItem>::const_iterator itr = itemList.begin(); itr != itemList.end(); itr++, i++) {
                ...
                jstring videoCover = env->NewStringUTF(itr->videoCover.c_str());
...
JNIEXPORT jlong JNICALL Java_com_qpidnetwork_livemodule_livechathttprequest_LCRequestJniLiveChat_GetVideoPhoto
  (JNIEnv *env, jclass, jstring user_sid, jstring user_id, jstring womanId, jstring videoid, jint size, jstring filePath, jobject callback) {
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::GetVideoPhoto() "
    		"requestId:%lld, user_sid:%s, user_id:%s, womanId:%s, videoid:%s, size:%d, filePath:%s"
    		, requestId, JString2String(env, user_sid).c_str(), JString2String(env, user_id).c_str(), JString2String(env, womanId).c_str(), JString2String(env, videoid).c_str(),
    		 size, JString2String(env, filePath).c_str());
...
JNIEXPORT jlong JNICALL Java_com_qpidnetwork_livemodule_livechathttprequest_LCRequestJniLiveChat_GetVideo
  (JNIEnv *env, jclass, jstring user_sid, jstring user_id, jstring womanId, jstring videoid, jstring inviteid, jint toflag, jstring sendid, jobject callback) {
	...
	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::GetVideo() "
    		"requestId:%lld, user_sid:%s, user_id:%s, womanId:%s, videoid:%s, inviteid:%s"
    		, requestId, JString2String(env, user_sid).c_str(),
    		JString2String(env, user_id).c_str(), JString2String(env, womanId).c_str(),
    		JString2String(env, videoid).c_str(),
...
void RequestLiveChatControllerCallback::OnUploadManPhoto(long requestId, bool success, int errnum, const string& errmsg, const string& url, const string& md5)
{
	JNIEnv* env = NULL;
	bool isAttachThread = false;
	GetEnv(&env, &isAttachThread);

	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::OnUploadManPhoto( success : %s, env:%p, isAttachThread:%d )", success?"true":"false", env, isAttachThread);
	...
	string signure = "(JZILjava/lang/String;Ljava/lang/String;Ljava/lang/String;)V";
	jmethodID jCallback = env->GetMethodID(jCallbackCls, "OnUploadManPhoto", signure.c_str());


	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::OnUploadManPhoto(), errnum:%d, errmsg:%s, url:%s", errnum, errmsg.c_str(), url.c_str());
	if( jCallbackObj != NULL && jCallback != NULL ) {
		...
		FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::OnUploadManPhoto() jCallbackObj:%p, jCallback:%p, requestId:%ld, errnum:%d, jerrmsg:%p, jUrl:%p", jCallbackObj, jCallback, requestId, errnum, jerrmsg, jUrl);
		env->CallVoidMethod(jCallbackObj, jCallback, jlrequestId, success, errType, jerrmsg, jUrl, jmd5);
...
JNIEXPORT jlong JNICALL Java_com_qpidnetwork_livemodule_livechathttprequest_LCRequestJniLiveChat_UploadManPhoto
  (JNIEnv *env, jclass cls, jstring file, jobject callback) {

	jlong requestId = -1;

	string strFile = JString2String(env, file);

	requestId = gLSLiveChatRequestLiveChatController.UploadManPhoto(strFile);

	jobject obj = env->NewGlobalRef(callback);
	gCallbackMap.Insert(requestId, obj);

	FileLog(LIVESHOW_HTTP_LOG, "LiveChat.Native::UploadManPhoto() "
        	"requestId:%lld, file:%s"
        	, requestId, strFile.c_str());
