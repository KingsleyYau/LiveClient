...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private static final int STYLE_RED_CIRCLE_RIGHT_TOP = 3;   // zy2
    ...
    // 2019/8/29 zy2
    // https://github.com/H07000223/FlycoTabLayout/pull/165/commits/51129bfc462fe35d0a96b30cc3aca2805bbbee7e
    ...
    // 2019/8/29 zy2 指示器为图片
    // https://github.com/H07000223/FlycoTabLayout/pull/165/commits/9df626db4eb8e5cfc37b785656ae6aa131bf0c11
    ...
    private float emptyCircleRadius;

    private void initEmptyCirclePaint() {
        emptyCircleRadius = dp2px(3);

        emptyCirclePaint = new Paint();
        emptyCirclePaint.setColor(mIndicatorColor);
        emptyCirclePaint.setDither(true);
        emptyCirclePaint.setAntiAlias(true);
        // 2018/4/12  zy2   圆环选中点的样式设置
        emptyCirclePaint.setStyle(Paint.Style.STROKE);
        emptyCirclePaint.setStrokeWidth(5);
    }
...
//        Log.i("info", "onPageScrolled-----> position: " + position);
        /**
         * position:当前View的位置
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
        ...
        } else if (mIndicatorStyle == STYLE_RED_CIRCLE_RIGHT_TOP) {     // 2018/4/13 zy2
            int cx = 0;
            int cy = 0;
            TextView textView = getTitleView(mCurrentTab);
            cy = height / 2 - textView.getHeight() / 2 + textView.getHeight() / 4;
            cx = paddingLeft + mIndicatorRect.left + (mIndicatorRect.right - mIndicatorRect.left) / 2 + (mIndicatorRect.right - mIndicatorRect.left) / 3;

            canvas.drawCircle(cx, cy, emptyCircleRadius, emptyCirclePaint);
        } else {
            ...
            // 2018/5/9 zy2   设置指示器为图片(旧设置方式)
//            if (mBitmapDrawable != null) {
//                if (mIndicatorHeight > 0) {
//                    mBitmapDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
//                            height - mBitmapDrawable.getIntrinsicHeight() / 5 * 4 - (int) mIndicatorMarginBottom,
...
//            }
            ...
            if (mIndicatorHeight > 0) {
                // old
//                mIndicatorDrawable.setColor(mIndicatorColor);
                // 2019/3/13 zy2  设置了渐变条
                if (isIndicatorGradient) {
...
//                mIndicatorDrawable.draw(canvas);
                // 2019/8/29 zy2 指示器为图片
                if (mIndicatorSrc != 0) {
                    //设置了指示器为图片,暂时只有底部指示器样式
                    if (mBitmapDrawable == null) {
                        mBitmapDrawable = (BitmapDrawable) getResources().getDrawable(mIndicatorSrc);
                    }
                    mIndicatorHeight = mBitmapDrawable.getBitmap().getHeight();
                    mBitmapDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
    ...
    public void setIndicatorColor(int indicatorColor) {
        ...
        // 2019/3/13 zy2
        isIndicatorGradient = false;

        invalidate();
    }

    // 2019/3/13 zy2
    private boolean isIndicatorGradient;
    public void setIndicatorGradientColor(int[] colors){
        this.mIndicatorColors = colors;
        isIndicatorGradient = true;
        invalidate();
    }
    ...
    public void showStar(@DrawableRes int resId, int position, float leftPadding, final float bottomPadding){
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }

        View tabView = mTabsContainer.getChildAt(position);
        ImageView tipView = (ImageView) tabView.findViewById(R.id.iv_tab_star);
        if (tipView != null) {
            tipView.setImageResource(resId);

            if (tipView.getVisibility() == VISIBLE) {
                return;
            }

            tipView.setVisibility(VISIBLE);
            setStarMargin(position, leftPadding, bottomPadding);
        }
    }
    private void setStarMargin(int position, float leftPadding, final float bottomPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        final View tabView = mTabsContainer.getChildAt(position);
        final ImageView tipView = (ImageView) tabView.findViewById(R.id.iv_tab_star);
        if (tipView != null) {
            TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            mTextPaint.setTextSize(mTextsize);
...
//            lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight) / 2 - dp2px(bottomPadding) : 0;
            ...
            tipView.setPadding(tipView.getPaddingLeft(), dp2px(bottomPadding), tipView.getPaddingRight(), tipView.getPaddingBottom());
        }
    }
