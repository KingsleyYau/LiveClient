...
//    		        }
	...
	private int loadGLShader(int type, String shaderString) {
		...
		if( shaderString != null ) {
	        ...
	        if ( compiled[0] == 0 ) {
	        	Log.e(LSConfig.TAG, String.format("LSShaderUtil::loadGLShader( [Compile Shader Failed] : %s )", GLES20.glGetShaderInfoLog(shader)));
	        	shader = -1;
	        } else {
//	        	Log.d(LSConfig.TAG, String.format("LSShaderUtil::loadShader(\n%s\n) ", shaderString));
			}
	        
		} else {
			Log.e(LSConfig.TAG, String.format("LSShaderUtil::loadGLShader( [Load Shader String Failed] )"));
		}
