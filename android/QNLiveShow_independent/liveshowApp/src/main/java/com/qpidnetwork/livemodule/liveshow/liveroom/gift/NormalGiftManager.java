...
//                        Log.d(TAG,"onGetGiftList-giftList.length:"+giftList.length);
     ...
     * @param callback
     */
    public void getSendableGiftItems(final String roomid,final OnGetSendableGiftListCallback callback){
        currRoomId = roomid;
        //一般不存在房间可发送礼物为空的情况
        ...
        LiveRequestOperator.getInstance().GetSendableGiftList(roomid, new OnGetSendableGiftListCallback() {
            @Override
            public void onGetSendableGiftList(boolean isSuccess, int errCode,
                ...
                roomSendableGiftReqStatusList.put(roomid,isSuccess ?
                        HttpReqStatus.ReqSuccess : HttpReqStatus.ResFailed);
                updateRooomSendableGift();
                if(null != callback){
                    ...
                    getGiftDetail(sendableGiftItem.giftId, new OnGetGiftDetailCallback() {
                        ...
                        public void onGetGiftDetail(boolean isSuccess, int errCode, String errMsg, GiftItem giftDetail) {
                            ...
                            if(roomShowSendableGiftItems.size() == currRoomShowableGiftNum){
                                OnRoomShowSendableGiftDataChangeListener listener = listeners.get(currRoomId);
                                if(null != listener){
                                    listener.onRoomSendableGiftChanged(currRoomId,roomShowSendableGiftItems);
                                }
...
//                                Log.d(TAG,"图片下载成功,localPath:"+localPath);
    ...
    public List<GiftItem> getLocalSendabelRecomGiftList(String roomId, int manLevel, int loveLevel){
        ...
        if(sendableGiftItems != null){
            ...
            //极端情况，默认推荐第一页第一个gift
            if(0 == giftList.size()){
                SendableGiftItem firstSGItem = sendableGiftItems.get(0);
                if(allGiftItems.containsKey(firstSGItem.giftId)){
                    GiftItem giftItem = allGiftItems.get(firstSGItem.giftId);
                    giftList.add(giftItem);
                }
            }
        }
