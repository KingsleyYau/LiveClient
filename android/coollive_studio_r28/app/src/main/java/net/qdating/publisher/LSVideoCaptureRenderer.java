...
public class LSVideoCaptureRenderer implements Renderer, LSImageRecordFilterCallback {
    ...
    private boolean bCustomPreViewFilterChange = false;
    private LSImageFilter customPreviewFilter = null;
    ...
    public LSVideoCaptureRenderer(ILSVideoPreviewCallback callback, FillMode fillMode, boolean useHardEncoder, LSPublishConfig publishConfig) {
        ...
        if (recordFilter != null) {
            recordFilter.setOutputSize(publishConfig.videoWidth, publishConfig.videoHeight);
        }
    }
     ...
     * @param customFilter 自定义滤镜
     */
    public void setCustomPreviewFilter(LSImageFilter customFilter) {
        Log.d(LSConfig.TAG, String.format("LSVideoCaptureRenderer::setCustomPreviewFilter( this : 0x%x )", hashCode()));
        synchronized (this) {
            if (this.customPreviewFilter != customFilter) {
                this.customPreviewFilter = customFilter;
                bCustomPreViewFilterChange = true;

                if (this.customPreviewFilter != null) {
                    outputFlipFilter.setFilter(this.customPreviewFilter);
                    this.customPreviewFilter.setFilter(this.outputFilter);
                } else {
                    outputFlipFilter.setFilter(this.outputFilter);
                }
            }
        }
    }
    ...
    public void changeCropFilterSize(int inputWidth, int inputHeight) {
        ...
        if (inputHeight == 0 || publishConfig.videoHeight == 0 || radioPreview == radioImage) {
            // 不裁剪
            cropFilter.setCropRect(0, 0, 1, 1);
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized (this) {
            ...
            if (bCustomPreViewFilterChange) {
                if (customPreviewFilter != null) {
                    customPreviewFilter.init();
                }
                bCustomPreViewFilterChange = false;
            }
