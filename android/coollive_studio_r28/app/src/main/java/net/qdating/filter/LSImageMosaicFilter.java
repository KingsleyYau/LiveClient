 ...
 *
 */
public class LSImageMosaicFilter extends LSImageBufferFilter {
	...
	private static String fragmentShaderString = ""
			...
			+ "uniform lowp vec2 uImageSize; \n"
			+ "// 片段着色器(输入) \n"
			+ "varying highp vec2 vTextureCoordinate; \n"
			+ "void main() { \n"
			+ "		vec2 xy = vTextureCoordinate.xy; \n "
			+ "		float numerator = uImageSize.x; \n "
			+ "		float denominator = uImageSize.y; \n "
			+ "		vec2 newXy = vec2(floor(xy.x * numerator) * denominator, floor(xy.y * numerator) * denominator); \n "
			+ "		gl_FragColor = texture2D(uInputTexture, newXy); \n"
			+ "} \n";
	...
	private float ufNumerator;
	private float ufDenominator;

	public LSImageMosaicFilter() {
		super(vertexShaderString, fragmentShaderString, LSImageVertex.filterVertex_0);
		// TODO Auto-generated constructor stub
		this.level = 0.2f;
	}

	public LSImageMosaicFilter(float level) {
		super(vertexShaderString, fragmentShaderString, LSImageVertex.filterVertex_0);
		// TODO Auto-generated constructor stub
		this.level = level;
	}
	 ...
	 * @param level 0.0 ~ 1.0
	 */
	public void setMosaicLevel(float level) {
		this.level = level;
	}
	...
	protected ImageSize changeInputSize(int inputWidth, int inputHeight) {
		...
		if( imageSize.bChange ) {
			radius = inputWidth / 10 * level;
			ufNumerator = inputWidth / radius;
			ufDenominator = radius / inputWidth;
		}
	...
	protected void onDrawStart(int textureId) {
		...
		uImageSize = GLES20.glGetUniformLocation(getProgram(), "uImageSize");
		...
		float uvImageSize[] = {ufNumerator, ufDenominator};
		GLES20.glUniform2f(uImageSize, uvImageSize[0], uvImageSize[1]);
	}
