...
public class LSVideoHardDecoder implements ILSVideoHardDecoderJni {
    ...
    static public boolean isHardCodec(String codecName) {
        // 判断是否为硬解码：排除google软解，且以OMX.开头（硬解码组件特征）
        if (!codecName.startsWith("OMX.google.")
                && codecName.startsWith("OMX.")
                && !codecName.contains("software")) {
            return true; // 返回硬解码器名称
        }
        return false;
    }
...
//                MediaCodecInfo codecInfo = MediaCodecList.getCodecInfoAt(i);
        		...
        		for (int j = 0; j < supportTypes.length; j++) {
        			...
        			if( !codecInfo.isEncoder() &&
                            supportTypes[j].equalsIgnoreCase(MIME_TYPE) &&
                            isHardCodec(codecInfo.getName()) ) {
                        Log.i(LSConfig.TAG,
                                String.format("LSVideoHardDecoder::supportHardDecoder( "
