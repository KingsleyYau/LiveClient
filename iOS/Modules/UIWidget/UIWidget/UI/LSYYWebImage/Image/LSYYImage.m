...
+ (LSYYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (LSYYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (LSYYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        LSYYImageDecoder *decoder = [LSYYImageDecoder decoderWithData:data scale:scale];
        LSYYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"LSYYImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"LSYYImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"LSYYImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"LSYYImageData"];
    } else {
