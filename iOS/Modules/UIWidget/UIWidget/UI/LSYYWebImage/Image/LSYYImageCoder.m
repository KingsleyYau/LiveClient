...
#import "webp/mux.h"
#else
#define LSYYImage_WEBP_ENABLED 0
#endif
#endif
...
static inline size_t LSYYImageByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat LSYYImageDegreesToRadians(CGFloat degrees) {
...
CGImageRef YYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    ...
    size_t destBytesPerRow = LSYYImageByteAlign(destWidth * 4, 32);
...
CGImageRef YYCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(LSYYImageDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(LSYYImageDegreesToRadians(90));
...
LSYYImageType LSYYImageDetectType(CFDataRef data) {
    if (!data) return LSYYImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return LSYYImageTypeUnknown;
    ...
    switch (magic4) {
        ...
        case YY_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return LSYYImageTypeICNS;
        } break;
            
        case YY_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return LSYYImageTypeGIF;
        } break;
            
        case YY_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == YY_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return LSYYImageTypePNG;
            }
        } break;
        ...
        case YY_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            ...
            if (tmp == YY_FOUR_CC('W', 'E', 'B', 'P')) {
                return LSYYImageTypeWebP;
            }
        } break;
        /*
        case YY_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
            return LSYYImageTypeBPG;
        } break;
        */
    }
    ...
    switch (magic2) {
        ...
        case YY_TWO_CC('C', 'P'): { // BMP
            return LSYYImageTypeBMP;
        }
        case YY_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return LSYYImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return LSYYImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return LSYYImageTypeJPEG2000;
    
    return LSYYImageTypeUnknown;
}

CFStringRef LSYYImageTypeToUTType(LSYYImageType type) {
    switch (type) {
        case LSYYImageTypeJPEG: return kUTTypeJPEG;
        case LSYYImageTypeJPEG2000: return kUTTypeJPEG2000;
        case LSYYImageTypeTIFF: return kUTTypeTIFF;
        case LSYYImageTypeBMP: return kUTTypeBMP;
        case LSYYImageTypeICO: return kUTTypeICO;
        case LSYYImageTypeICNS: return kUTTypeAppleICNS;
        case LSYYImageTypeGIF: return kUTTypeGIF;
        case LSYYImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

LSYYImageType LSYYImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(LSYYImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(LSYYImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(LSYYImageTypeTIFF),
                (id)kUTTypeBMP : @(LSYYImageTypeBMP),
                (id)kUTTypeICO : @(LSYYImageTypeICO),
                (id)kUTTypeAppleICNS : @(LSYYImageTypeICNS),
                (id)kUTTypeGIF : @(LSYYImageTypeGIF),
                (id)kUTTypePNG : @(LSYYImageTypePNG)};
    });
    if (!uti) return LSYYImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *LSYYImageTypeGetExtension(LSYYImageType type) {
    switch (type) {
        case LSYYImageTypeJPEG: return @"jpg";
        case LSYYImageTypeJPEG2000: return @"jp2";
        case LSYYImageTypeTIFF: return @"tiff";
        case LSYYImageTypeBMP: return @"bmp";
        case LSYYImageTypeICO: return @"ico";
        case LSYYImageTypeICNS: return @"icns";
        case LSYYImageTypeGIF: return @"gif";
        case LSYYImageTypePNG: return @"png";
        case LSYYImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef YYCGImageCreateEncodedData(CGImageRef imageRef, LSYYImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == LSYYImageTypeWebP) {
#if LSYYImage_WEBP_ENABLED
        if (quality == 1) {
            return YYCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, LSYYImagePresetDefault);
        } else {
            return YYCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, LSYYImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = LSYYImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL LSYYImageWebPAvailable() {
    return YES;
}

CFDataRef YYCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, LSYYImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > LSYYImagePresetText ? LSYYImagePresetDefault : preset;
...
NSUInteger LSYYImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef YYCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    bitsPerComponent = 8;
    bitsPerPixel = 32;
    bytesPerRow = LSYYImageByteAlign(bitsPerPixel / 8 * canvasWidth, 32);
...
@implementation LSYYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    LSYYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    LSYYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _LSYYImageDecoderFrame : LSYYImageFrame
...
@implementation _LSYYImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _LSYYImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation LSYYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    LSYYImageDecoder *decoder = [[LSYYImageDecoder alloc] initWithScale:scale];
...
- (LSYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    LSYYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_LSYYImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    LSYYImageType type = LSYYImageDetectType((__bridge CFDataRef)data);
...
- (LSYYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _LSYYImageDecoderFrame *frame = [(_LSYYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != LSYYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == LSYYImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = LSYYImageDisposeNone;
        frame.blend = LSYYImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSource {
    switch (_type) {
        case LSYYImageTypeWebP: {
            [self _updateSourceWebP];
        } break;
            
        case LSYYImageTypePNG: {
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _LSYYImageDecoderFrame *frame = [_LSYYImageDecoderFrame new];
            [frames addObject:frame];
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = LSYYImageDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = LSYYImageBlendOver;
            }
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/ibireme/LSYYImage#installation", func, line);
    });
#endif
}
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        switch (fi->frame_control.dispose_op) {
            ...
            case YY_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = LSYYImageDisposePrevious;
            } break;
            default: {
                frame.dispose = LSYYImageDisposeNone;
            } break;
        ...
        if (frame.blend == LSYYImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != LSYYImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == LSYYImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == LSYYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == LSYYImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _LSYYImageDecoderFrame *frame = _frames[index];
    ...
    if (_webpSource) {
        ...
        size_t bytesPerRow = LSYYImageByteAlign(bitsPerPixel / 8 * width, 32);
...
- (void)_blendImageWithFrame:(_LSYYImageDecoderFrame *)frame {
    if (frame.dispose == LSYYImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == LSYYImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == LSYYImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_LSYYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == LSYYImageDisposePrevious) {
        if (frame.blend == LSYYImageBlendOver) {
...
@implementation LSYYImageEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"LSYYImageEncoder init error" reason:@"LSYYImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
    return [self initWithType:LSYYImageTypeUnknown];
}

- (instancetype)initWithType:(LSYYImageType)type {
    if (type == LSYYImageTypeUnknown || type >= LSYYImageTypeOther) {
...
#if !LSYYImage_WEBP_ENABLED
    if (type == LSYYImageTypeWebP) {
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/ibireme/LSYYImage#installation", __FUNCTION__, __LINE__);
        return nil;
    }
#endif
    ...
    switch (type) {
        case LSYYImageTypeJPEG:
        case LSYYImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case LSYYImageTypeTIFF:
        case LSYYImageTypeBMP:
        case LSYYImageTypeGIF:
        case LSYYImageTypeICO:
        case LSYYImageTypeICNS:
        case LSYYImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, LSYYImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, LSYYImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == LSYYImageTypeGIF && count > 1) {
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == LSYYImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = YYCGImageCreateEncodedData(decoded, LSYYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CFDataRef frameData = YYCGImageCreateEncodedData(extendedImage, LSYYImageTypePNG, 1);
        if (!frameData) {
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = YYCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, LSYYImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == LSYYImageTypePNG) return [self _encodeAPNG];
    if (_type == LSYYImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(LSYYImageType)type quality:(CGFloat)quality {
    LSYYImageEncoder *encoder = [[LSYYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(LSYYImageDecoder *)decoder type:(LSYYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (LSLSYYImageCoder)
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[LSYYSpriteSheetImage class]]) return YES;
...
/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for LSYYImage (PNG/JPEG/GIF/WebP)
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[LSYYImage class]]) {
        LSYYImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == LSYYImageTypeGIF ||
                    image.animatedImageType == LSYYImageTypePNG) {
