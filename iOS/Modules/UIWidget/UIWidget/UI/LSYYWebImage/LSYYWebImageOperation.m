...
@property (nonatomic, strong) LSYYImageDecoder *progressiveDecoder;
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[LSYYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == LSYYImageTypeUnknown ||
            _progressiveDecoder.type == LSYYImageTypeWebP ||
            _progressiveDecoder.type == LSYYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != LSYYImageTypeJPEG &&
                _progressiveDecoder.type != LSYYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            LSYYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
            ...
            } else if (_progressiveDecoder.type == LSYYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                if (allowAnimation) {
                    image = [[LSYYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((LSYYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    LSYYImageDecoder *decoder = [LSYYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                LSYYImageType imageType = LSYYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case LSYYImageTypeJPEG:
                    case LSYYImageTypeGIF:
                    case LSYYImageTypePNG:
                    case LSYYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == LSYYImageTypeGIF ||
                                imageType == LSYYImageTypeWebP) {
