...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, LSYYImageType) {
    LSYYImageTypeUnknown = 0, ///< unknown
    LSYYImageTypeJPEG,        ///< jpeg, jpg
    LSYYImageTypeJPEG2000,    ///< jp2
    LSYYImageTypeTIFF,        ///< tiff, tif
    LSYYImageTypeBMP,         ///< bmp
    LSYYImageTypeICO,         ///< ico
    LSYYImageTypeICNS,        ///< icns
    LSYYImageTypeGIF,         ///< gif
    LSYYImageTypePNG,         ///< png
    LSYYImageTypeWebP,        ///< webp
    LSYYImageTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, LSYYImageBlendOperation) {
    
    /**
     All color components of the frame, including alpha, overwrite the current
     contents of the frame's canvas region.
     */
    LSYYImageBlendNone = 0,
 ...
 */
@interface LSYYImageFrame : NSObject <NSCopying>
...
@property (nonatomic) LSYYImageDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) LSYYImageBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    LSYYImageDecoder *decoder = [LSYYImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    LSYYImageDecoder *decoder = [[LSYYImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface LSYYImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) LSYYImageType type;          ///< Image data type.
 ...
 */
- (nullable LSYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in LSYYImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    LSYYImageEncoder *jpegEncoder = [[LSYYImageEncoder alloc] initWithType:LSYYImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    LSYYImageEncoder *gifEncoder = [[LSYYImageEncoder alloc] initWithType:LSYYImageTypeGIF];
 ...
 */
@interface LSYYImageEncoder : NSObject

@property (nonatomic, readonly) LSYYImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(LSYYImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(LSYYImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(LSYYImageDecoder *)decoder type:(LSYYImageType)type quality:(CGFloat)quality;
...
@interface UIImage (LSLSYYImageCoder)
...
CG_EXTERN LSYYImageType LSYYImageDetectType(CFDataRef data);

/// Convert LSYYImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable LSYYImageTypeToUTType(LSYYImageType type);

/// Convert UTI (such as kUTTypeJPEG) to LSYYImageType.
CG_EXTERN LSYYImageType LSYYImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable LSYYImageTypeGetExtension(LSYYImageType type);
 ...
 */
CG_EXTERN CFDataRef _Nullable YYCGImageCreateEncodedData(CGImageRef imageRef, LSYYImageType type, CGFloat quality);


/**
 Whether WebP is available in LSYYImage.
 */
CG_EXTERN BOOL LSYYImageWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger LSYYImageGetWebPFrameCount(CFDataRef webpData);
...
typedef NS_ENUM(NSUInteger, LSYYImagePreset) {
    LSYYImagePresetDefault = 0,  ///< default preset.
    LSYYImagePresetPicture,      ///< digital picture, like portrait, inner shot
    LSYYImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    LSYYImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    LSYYImagePresetIcon,         ///< small-sized colorful images
    LSYYImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is LSYYImagePresetDefault.
