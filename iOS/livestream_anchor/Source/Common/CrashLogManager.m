...
#pragma mark - 上传日志
- (void)uploadCrashLog:(NSString *)srcDirectory tmpDirectory:(NSString *)tmpDirectory {
    [self.requestManager anchorUploadCrashFile:[UIDevice currentDevice].identifierForVendor.UUIDString file:srcDirectory tmpDirectory:tmpDirectory finishHandler:^(BOOL success, ZBHTTP_LCC_ERR_TYPE errnum, NSString * _Nonnull errmsg) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
