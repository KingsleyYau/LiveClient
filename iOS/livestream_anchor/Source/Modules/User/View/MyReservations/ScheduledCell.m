...
+ (id)getUITableViewCell:(UITableView *)tableView forRow:(NSInteger)row {
    ScheduledCell *cell = (ScheduledCell *)[tableView dequeueReusableCellWithIdentifier:[ScheduledCell cellIdentifier]];
    
    if (nil == cell) {
        NSArray *nib = [[LiveBundle mainBundle] loadNibNamedWithFamily:[ScheduledCell cellIdentifier] owner:tableView options:nil];
...
- (IBAction)startNowDid:(UITapGestureRecognizer *)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(scheduledStartNowDidForRow:)]) {
        [self.delegate scheduledStartNowDidForRow:sender.view.tag];
    }
}

- (IBAction)headTap:(UITapGestureRecognizer *)sender {
    
    if ([self.delegate respondsToSelector:@selector(scheduledHeadDidForRow:)]) {
        [self.delegate scheduledHeadDidForRow:sender.view.tag];
    }
}
...
- (void)countdown {
    ...
    if (self.time == self.countdownTime) {
        if ([self.delegate respondsToSelector:@selector(scheduledCountdownEnd)]) {
            [self.delegate scheduledCountdownEnd];
        }
    }
}
