...
+ (NSString* _Nonnull)getScreenNameWithName:(NSString* _Nonnull)name
{
    ...
    // 获取屏幕名称
    NSString* screenName = NSLocalizedStringFromTable(name, @"LSGoogleAnalyticsScreenName", nil);
...
+ (NSString* _Nonnull)getHomeScreenName {
    // 获取屏幕名称
    NSString* screenName = NSLocalizedStringFromTable(@"LSMainViewController", @"LSGoogleAnalyticsScreenName", nil);
    return screenName;
}
...
+ (NSString* _Nonnull)getPageName:(UIViewController* _Nonnull)viewController pageIndex:(NSUInteger)pageIndex
{
    // 获取屏幕名称（没有则使用类名）
    NSString* className = [[viewController class] description];
    NSString* mainName = NSLocalizedStringFromTable(className, @"LSGoogleAnalyticsScreenName", nil);
    // 组成带页号的屏幕名称
