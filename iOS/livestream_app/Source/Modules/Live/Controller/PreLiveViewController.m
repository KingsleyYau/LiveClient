    ...
    if (self.exitLeftSecond > 0) {
        ...
        if( self.status != PreLiveStatus_Error ) {
            if (success) {
                // 请求进入成功
                // 更新本地登录信息
                [LSLoginManager manager].loginItem.userlevel = roomItem.manLevel;
...
- (IBAction)retry:(id)sender {
    // TODO:点击重试
    [[LSAnalyticsManager manager] reportActionEvent:BroadcastTransitionClickContinue eventCategory:EventCategoryTransition];
    // 重置参数
    [self reset];
...
- (IBAction)startVipPrivate:(id)sender {
    [[LSAnalyticsManager manager] reportActionEvent:BroadcastTransitionClickStartVip eventCategory:EventCategoryTransition];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    //TODO:点击推荐主播
    [[LSAnalyticsManager manager] reportActionEvent:BroadcastTransitionClickRecommend eventCategory:EventCategoryTransition];
