...
- (void)login {
    NSLog(@"LSImManager::login( [IM登陆], token : %@ )", self.loginManager.token);

    // 开始登录IM
    [self.client login:self.loginManager.token pageName:PAGENAMETYPE_MOVEPAGE];
}
...
- (void)onLogin:(LCC_ERR_TYPE)errType errMsg:(NSString *_Nonnull)errmsg item:(ImLoginReturnObject *_Nonnull)item {
    NSLog(@"LSImManager::onLogin( [IM登录, %@], errType : %d, errmsg : %@ )", (errType == LCC_ERR_SUCCESS) ? @"成功" : @"失败", errType, errmsg);
