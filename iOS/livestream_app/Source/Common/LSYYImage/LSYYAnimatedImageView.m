 ...
 */
@interface _LSYYImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _LSYYImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_LSYYImageWeakProxy alloc] initWithTarget:target];
}
...
@interface LSYYAnimatedImageView() {
    @package
...
@interface _LSYYAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) LSYYAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <YYAnimatedImage> *curImage;
@end

@implementation _LSYYAnimatedImageViewFetchOperation
- (void)main {
    __strong LSYYAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation LSYYAnimatedImageView
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[_LSYYImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)stopAnimating {
    ...
    if (_totalFrameCount-1 == _curIndex && self.isOneRunloop) {
        _curIndex = 0;
        _totalFrameCount = 0;
        self.image = nil;
...
- (void)step:(CADisplayLink *)link {
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _LSYYAnimatedImageViewFetchOperation *operation = [_LSYYAnimatedImageViewFetchOperation new];
        operation.view = self;
