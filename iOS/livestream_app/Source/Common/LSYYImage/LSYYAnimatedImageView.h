...
/**
 设置动画是否循环播放
 */
@property (nonatomic) BOOL isOneRunloop;
...
/**
 ...
 display with LSYYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to LSYYAnimatedImageView.image or LSYYAnimatedImageView.highlightedImage
 to display animation.
 
 See `LSYYImage` and `LSYYFrameImage` for example.
 */
