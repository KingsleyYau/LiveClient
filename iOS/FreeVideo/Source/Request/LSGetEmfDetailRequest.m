...
@implementation LSGetEmfDetailRequest
- (instancetype)init{
    if (self = [super init]) {
        self.emfId = @"";
    }
...
- (BOOL)sendRequest {
    if( self.manager ) {
        ...
        NSInteger request = [self.manager getEmfDetail:self.emfId finishHandler:^(BOOL success, HTTP_LCC_ERR_TYPE errnum, NSString *errmsg, LSHttpLetterDetailItemObject *item) {
            BOOL bFlag = NO;
