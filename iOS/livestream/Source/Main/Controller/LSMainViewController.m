...
- (void)liveUrlHandlerOpenLiveChatList:(LiveUrlHandler *)handler {
    // TODO::进入聊天界面
    NSLog(@"LSMainViewController::liveUrlHandlerOpenLiveChatList ( [URL跳转,进入聊天列表] )");
...
- (void)liveUrlHandler:(LiveUrlHandler *)handler openSayHiType:(LiveUrlSayHiListType)type {
    ...
    NSLog(@"LSMainViewController::liveUrlHandler( [URL跳转, sayhi列表页], openSayHiType : %d, index : %d )", type, index);
...
- (void)liveUrlHandler:(LiveUrlHandler *)handler didOpenSayHiDetail:(NSString *)sayhiId {
    // TODO:进入sayhi详情
    NSLog(@"LSMainViewController::liveUrlHandler( [URL跳转, sayhi详情], didOpenSayHiDetail : %@,)", sayhiId);
    [[LiveGobalManager manager] popToRootVC];
