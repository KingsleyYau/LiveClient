...
@implementation CloseLiveRoomRequest
- (instancetype)init{
    if (self = [super init]) {
...
- (BOOL)sendRequest {
    if( self.manager ) {
        ...
        NSInteger request = [self.manager closeLiveRoom:self.token roomId:self.roomId finishHandler:^(BOOL success, NSInteger errnum, NSString * _Nonnull errmsg) {
            BOOL bFlag = NO;
