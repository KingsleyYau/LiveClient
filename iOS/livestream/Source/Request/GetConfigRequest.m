...
- (BOOL)sendRequest {
    if( self.manager ) {
        ...
        NSInteger request = [self.manager getConfig:^(BOOL success, NSInteger errnum, NSString * _Nonnull errmsg, ConfigItemObject * _Nullable item) {
            BOOL bFlag = NO;
...
- (void)callRespond:(BOOL)success errnum:(NSInteger)errnum errmsg:(NSString* _Nullable)errmsg {
    if( self.finishHandler && !success ) {
        ConfigItemObject *item = [[ConfigItemObject alloc] init];
