...
@implementation GetLiveRoomPhotoListRequest
- (instancetype)init{
    if (self = [super init]) {
...
- (BOOL)sendRequest {
    if( self.manager ) {
        ...
        NSInteger request = [self.manager getLiveRoomPhotoList:self.token finishHandler:^(BOOL success, NSInteger errnum, NSString * _Nonnull errmsg, NSArray<CoverPhotoItemObject *> * _Nullable array) {
            BOOL bFlag = NO;
