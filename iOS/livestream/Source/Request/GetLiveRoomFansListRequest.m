...
@implementation GetLiveRoomFansListRequest
- (instancetype)init{
    if (self = [super init]) {
...
- (BOOL)sendRequest {
    if( self.manager ) {
        ...
        NSInteger request = [self.manager getLiveRoomFansList:self.token roomId:self.roomId finishHandler:^(BOOL success, NSInteger errnum, NSString * _Nonnull errmsg, NSArray<ViewerFansItemObject *> * _Nullable array) {
            BOOL bFlag = NO;
