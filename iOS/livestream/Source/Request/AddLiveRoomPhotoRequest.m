...
@implementation AddLiveRoomPhotoRequest
- (instancetype)init{
    if (self = [super init]) {
...
- (BOOL)sendRequest {
    if( self.manager ) {
        ...
        NSInteger request = [self.manager addLiveRoomPhoto:self.token photoId:self.photoId finishHandler:^(BOOL success, NSInteger errnum, NSString * _Nonnull errmsg) {
            BOOL bFlag = NO;
