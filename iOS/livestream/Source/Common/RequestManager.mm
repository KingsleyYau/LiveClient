...
- (NSInteger)getAnchorList:(int)start
    ...
    NSInteger request = mHttpRequestController.GetAnchorList(&mHttpRequestManager, start, step, hasWatch, &gRequestGetAnchorListCallbackImp);
  ...
  public:
    ...
    void OnGetFollowList(HttpGetFollowListTask *task, bool success, int errnum, const string &errmsg, const FollowItemList &listItem) {
        ...
        for (FollowItemList::const_iterator iter = listItem.begin(); iter != listItem.end(); iter++) {
            ...
            item.anchorType = (*iter).anchorType;
            [array addObject:item];
            NSLog(@"RequestManager::OnGetLiveRoomHot( task : %p, userId : %@, nickName : %@, photoUrl : %@ , roomPhotoUrl : %@ , onlineStatus : %d , roomType : %d ", task, item.userId, item.nickName, item.photoUrl, item.roomPhotoUrl, item.onlineStatus, item.roomType);
        }
...
- (NSInteger)liveFansList:(NSString *_Nonnull)roomId
    ...
    NSInteger request = mHttpRequestController.LiveFansList(&mHttpRequestManager, [roomId UTF8String], start, step, &gLiveFansListCallbackImp);
...
- (NSInteger)getPromoAnchorList:(int)number
    ...
    NSInteger request = mHttpRequestController.GetPromoAnchorList(&mHttpRequestManager, number, type, [userId UTF8String], &gRequestGetPromoAnchorListCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
...
- (NSInteger)sendBookingRequest:(NSString *_Nullable)userId
    ...
    NSInteger request = mHttpRequestController.SendBookingRequest(&mHttpRequestManager, [userId UTF8String], [timeId UTF8String], bookTime, [giftId UTF8String], giftNum, needSms, &gRequestSendBookingRequestCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
...
class RequestAcceptInstanceInviteCallbackImp : public IRequestAcceptInstanceInviteCallback {
  public:
    ...
    void OnAcceptInstanceInvite(HttpAcceptInstanceInviteTask *task, bool success, int errnum, const string &errmsg, const HttpAcceptInstanceInviteItem &item) {
        NSLog(@"RequestManager::OnAcceptInstanceInvite( task : %p, success : %s, errnum : %d, errmsg : %s)", task, success ? "true" : "false", errnum, errmsg.c_str());
...
- (NSInteger)acceptInstanceInvite:(NSString *_Nullable)inviteId
    ...
    NSInteger request = mHttpRequestController.AcceptInstanceInvite(&mHttpRequestManager, [inviteId UTF8String], isConfirm, &gRequestAcceptInstanceInviteCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
...
class RequestGetAdAnchorListCallbackImp : public IRequestGetAdAnchorListCallback {
  public:
    ...
    void OnGetAdAnchorList(HttpGetAdAnchorListTask *task, bool success, int errnum, const string &errmsg, const HotItemList &list) {
        NSLog(@"RequestManager::OnGetAdAnchorList( task : %p, success : %s, errnum : %d, errmsg : %s)", task, success ? "true" : "false", errnum, errmsg.c_str());
...
- (NSInteger)getAdAnchorList:(int)number
    ...
    NSInteger request = mHttpRequestController.GetAdAnchorList(&mHttpRequestManager, number, &gRequestGetAdAnchorListCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
...
class RequestCloseAdAnchorListCallbackImp : public IRequestCloseAdAnchorListCallback {
  public:
    ...
    void OnCloseAdAnchorList(HttpCloseAdAnchorListTask *task, bool success, int errnum, const string &errmsg) {
        NSLog(@"RequestManager::OnCloseAdAnchorList( task : %p, success : %s, errnum : %d, errmsg : %s)", task, success ? "true" : "false", errnum, errmsg.c_str());
...
- (NSInteger)closeAdAnchorList:(CloseAdAnchorListFinishHandler _Nullable)finishHandler {
    NSInteger request = mHttpRequestController.CloseAdAnchorList(&mHttpRequestManager, &gRequestCloseAdAnchorListCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
...
class RequestGetPhoneVerifyCodeCallbackImp : public IRequestGetPhoneVerifyCodeCallback {
  public:
    ...
    void OnGetPhoneVerifyCode(HttpGetPhoneVerifyCodeTask *task, bool success, int errnum, const string &errmsg) {
        NSLog(@"RequestManager::OnGetPhoneVerifyCode( task : %p, success : %s, errnum : %d, errmsg : %s)", task, success ? "true" : "false", errnum, errmsg.c_str());
...
- (NSInteger)getPhoneVerifyCode:(NSString *_Nonnull)country
    ...
    NSInteger request = mHttpRequestController.GetPhoneVerifyCode(&mHttpRequestManager, [country UTF8String], [areaCode UTF8String], [phoneNo UTF8String], &gRequestGetPhoneVerifyCodeCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
...
class RequestSubmitPhoneVerifyCodeCallbackImp : public IRequestSubmitPhoneVerifyCodeCallback {
  public:
    ...
    void OnSubmitPhoneVerifyCode(HttpSubmitPhoneVerifyCodeTask *task, bool success, int errnum, const string &errmsg) {
        NSLog(@"RequestManager::OnSubmitPhoneVerifyCode( task : %p, success : %s, errnum : %d, errmsg : %s)", task, success ? "true" : "false", errnum, errmsg.c_str());
...
- (NSInteger)submitPhoneVerifyCode:(NSString *_Nonnull)country
    ...
    NSInteger request = mHttpRequestController.SubmitPhoneVerifyCode(&mHttpRequestManager, [country UTF8String], [areaCode UTF8String], [phoneNo UTF8String], [verifyCode UTF8String], &gRequestSubmitPhoneVerifyCodeCallbackImp);
    if (request != HTTPREQUEST_INVALIDREQUESTID) {
        @synchronized(self.delegateDictionary) {
            [self.delegateDictionary setObject:finishHandler forKey:@((NSInteger)request)];
        }
