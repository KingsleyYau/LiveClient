...
- (void)viewDidLoad {
    ...
    // 加载观众头像
    [self.imageLoader loadImageWithImageView:self.headImageView options:SDWebImageRefreshCached
       imageUrl:self.loginManager.loginItem.photoUrl placeholderImage:[UIImage imageNamed:@"Default_Img_Noman_HangOut"]];
...
- (void)sendRequestManPush:(BOOL)isStart {
    ...
    BOOL bflag = [self.imManager controlManPushHangout:self.liveRoom.roomId control:type finishHandler:^(BOOL success, LCC_ERR_TYPE errType, NSString * _Nonnull errMsg, NSArray<NSString *> * _Nonnull manPushUrl) {
...
- (void)onRecvHangoutGiftNotice:(IMRecvHangoutGiftItemObject *)item {
    ...
    if (isEqualUserId && isEqualRoomId) {
        NSLog(@"HangOutUserViewController::onRecvHangoutGiftNotice( [接收多人互动直播间礼物通知] roomId : %@, fromId : %@, toUid : %@,"
              "giftId : %@, userId : %@ )",item.roomId, item.fromId, item.toUid, item.giftId, item.toUid);
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)onRecvEnterHangoutRoomNotice:(IMRecvEnterRoomItemObject *)item {
    ...
    if (!isAnchor && isEqualUserId && isEqualRoomId) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self.userInfoManager setAudienceInfoDicL:model];
        });
    }
}
