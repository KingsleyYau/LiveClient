...
@interface CeleBrationGiftLayout()
...
@implementation CeleBrationGiftLayout
- (instancetype)init {
    if (self = [super init]) {
        // 页数当前页的初始化
        self.currentPage = 0;
...
- (CGSize)collectionViewContentSize {
    ...
    if( (self.itemWidth + self.itemSpace) != 0 ) {
        self.lineItemCount = (([UIScreen mainScreen].bounds.size.width - self.itemWidth) / (self.itemWidth + self.itemSpace + self.paddingX)) + 1;
    }
