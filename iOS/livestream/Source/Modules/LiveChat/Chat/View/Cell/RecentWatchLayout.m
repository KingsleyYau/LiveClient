...
#import "RecentWatchLayout.h"

@interface RecentWatchLayout ()
...
@implementation RecentWatchLayout
- (instancetype)init {
    if (self = [super init]) {
        // 页数当前页的初始化
        self.currentPage = 0;
...
- (void)prepareLayout {
    ...
    self.itemSize = 0;
    self.itemWidth = self.collectionView.frame.size.width / 2;
    self.itemHeight = self.itemWidth * 0.56 + 33;
    self.itemSpace = 0;
    self.lineSpace = 0;
...
- (CGSize)collectionViewContentSize {
    ...
    CGSize size = CGSizeMake(superViewWidth, self.lineCount * self.itemHeight);
    return size;
}
