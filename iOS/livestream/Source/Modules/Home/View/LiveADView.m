...
- (void)loadAD {
    ...
    request.finishHandler = ^(BOOL success, HTTP_LCC_ERR_TYPE errnum, NSString *_Nonnull errmsg, NSString *_Nonnull bannerImg, NSString *_Nonnull bannerLink, NSString *_Nonnull bannerName) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                [self.imageLoader loadImageWithImageView:self.adView options:0 imageUrl:bannerImg placeholderImage:[UIImage imageNamed:@"Home_HotAndFollow_TableViewHeader_Banner"] finishHandler:nil];
