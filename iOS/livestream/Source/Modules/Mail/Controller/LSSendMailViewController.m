...
@interface LSSendMailViewController ()<LSSendMailPhotoViewDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,ChatTextViewDelegate,LSOutOfPoststampViewDelegate,LSOutOfCreditsViewDelegate>
...
- (void)dealloc {
    NSLog(@"LSSendMailViewController::dealloc()");
...
- (void)viewDidLoad {
    ...
    [[LSUserInfoManager manager]getLiverInfo:self.anchorId finishHandler:^(LSUserInfoModel * _Nonnull item) {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    BOOL userSendMailPriv = [LSLoginManager manager].loginItem.mailPriv.userSendMailPriv;
    if (userSendMailPriv) {
...
//        [self showAlertViewMsg:NSLocalizedStringFromSelf(@"SEND_MAIL_PERMISSOINS_TIP") cancelBtnMsg:NSLocalizedStringFromSelf(@"OK") otherBtnMsg:nil alertViewType:AlertViewTypeNoSendPermissions];
    ...
    if ([LiveRoomCreditRebateManager creditRebateManager].mPostStamp >= 1) {
        self.isStamps = YES;
        self.infoStr = [LSLoginManager manager].loginItem.mailPriv.userSendMailImgPriv.postStampMsg;
    }
    else {
        self.isStamps = NO;
        self.infoStr = [LSLoginManager manager].loginItem.mailPriv.userSendMailImgPriv.coinMsg;
    }
    
    self.fileArray = [NSMutableArray array];
    //图片上传数量
    self.uploadMaxNum = [LSLoginManager manager].loginItem.mailPriv.userSendMailImgPriv.maxImg;

    //没有发照片权限，隐藏控件
    if ([LSLoginManager manager].loginItem.mailPriv.userSendMailImgPriv.mailSendPriv == LSSENDIMGRISKTYPE_NOSEND || self.uploadMaxNum == 0) {
...
- (void)showAlertViewMsg:(NSString *)titleMsg cancelBtnMsg:(NSString *)cancelMsg otherBtnMsg:(NSString *)otherMsg alertViewType:(AlertViewType)type {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.56 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"" message:titleMsg delegate:self cancelButtonTitle:cancelMsg otherButtonTitles:otherMsg, nil];
        alertView.tag = type;
        [alertView show];
    });
}
