                ...
                request.finishHandler = ^(BOOL success, HTTP_LCC_ERR_TYPE errnum, NSString * _Nonnull errmsg, GetNewFansBaseInfoItemObject * _Nonnull item) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if(success) {
                            ...
                            NSLog(@"LSUserInfoManager:: getUserInfo: FansBaseInfo( userId : %@, photoUrl : %@ )", userId, item.photoUrl);
                            // 回调
                            if( finishHandler ) {
                                finishHandler(info);
                            }
        ...
        request.finishHandler = ^(BOOL success, HTTP_LCC_ERR_TYPE errnum, NSString * _Nonnull errmsg, LSUserInfoItemObject * _Nullable userInfoItem) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if(success) {
                    ...
                    NSLog(@"LSUserInfoManager::getLiverInfo( userId : %@, photoUrl : %@ )", userInfoItem.userId, userInfoItem.photoUrl);
                    // 回调
                    if( finishHandler ) {
                        finishHandler(info);
                    }
        ...
        request.finishHandler = ^(BOOL success, HTTP_LCC_ERR_TYPE errnum, NSString * _Nonnull errmsg, GetNewFansBaseInfoItemObject * _Nonnull item) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if(success) {
                    ...
                    NSLog(@"LSUserInfoManager::getFansBaseInfo( userId : %@, photoUrl : %@ )", userId, item.photoUrl);
                    // 回调
                    if( finishHandler ) {
                        finishHandler(info);
                    }
