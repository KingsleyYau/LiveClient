...
@interface LiveViewController () <UITextFieldDelegate, KKCheckButtonDelegate, BarrageViewDataSouce, BarrageViewDelegate, GiftComboViewDelegate, IMLiveRoomManagerDelegate, DriveViewDelegate>
...
- (void)initCustomParam {
    ...
    self.publishUrl = @"rtmp://172.25.32.17/live/max_i";
...
- (void)viewDidLoad {
    ...
    // 初始化返点按钮
    self.rebateBtn.layer.cornerRadius = self.rebateBtn.height / 2;
}
...
- (void)showPreview {
    // 显示预览控件
    self.previewVideoViewWidth.constant = 130;
}
...
- (void)setupGiftView {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        [giftComboView.backImageView setImage:[UIImage imageNamed:@"freeLive_combo_background"]];
...
- (BOOL)showCombo:(GiftItem *)giftItem giftComboView:(GiftComboView *)giftComboView withUserID:(NSString *)userId {
    ...
    //    [self.photoManager getLiveRoomUserPhotoRequestWithUserId:userId
    //                                                     andType:PHOTOTYPE_THUMB
    //                                                  requestEnd:^(NSString *userId, UserInfoItem *item) {
    //
    //                                                      // 请求用户头像 连击头像
    //                                                      [giftComboView.iconImageView sd_setImageWithURL:[NSURL URLWithString:item.userHeadUrl]
    //                                                                                     placeholderImage:[UIImage imageNamed:@"Login_Main_Logo"]
    //                                                                                              options:0];
    //                                                  }];
...
- (BOOL)sendMsg:(NSString *)text isLounder:(BOOL)isLounder {
    ...
    if (text.length > 0) {
        ...
        if (isLounder) {
            // 发送弹幕
            BarrageModel *item = [BarrageModel barrageModelForName:self.loginManager.loginItem.nickName
                                                           message:text
...
- (void)addChatMessageNickName:(NSString *)name userLevel:(NSInteger)level text:(NSString *)text {
    ...
    NSMutableAttributedString *attributeString = [self.msgManager presentTheSetStringItem:self.strItem msgItem:item];
...
- (void)addGiftMessageNickName:(NSString *)nickName giftID:(NSString *)giftID giftNum:(int)giftNum {
    ...
    //
    //    // 增加文本消息
    //    NSMutableAttributedString *attributeString = [self.msgManager presentTheSetStringItem:self.strItem msgItem:msgItem];
...
//        [msgCell setTableViewWidth:tableView.width];
...
- (void)onSendRoomToast:(BOOL)success reqId:(unsigned int)reqId errType:(LCC_ERR_TYPE)errType errMsg:(NSString *)errMsg coins:(double)coins {
    NSLog(@"LiveViewController::onSendRoomToast( [发送弹幕], success : %s, reqId : %d, errType : %d, errMsg : %@, coins : %f )", success ? "true" : "false", reqId, errType, errMsg, coins);
...
- (void)onRecvRoomMsg:(NSString *_Nonnull)roomId level:(int)level fromId:(NSString *_Nonnull)fromId nickName:(NSString *_Nonnull)nickName msg:(NSString *_Nonnull)msg {
    NSLog(@"LiveViewController::onRecvRoomMsg( [接收消息], roomId : %@, level : %d, fromId : %@, nickName : %@, msg : %@ )", roomId, level, fromId, nickName, msg);
...
- (void)onRecvPushRoomFav:(NSString *)roomId fromId:(NSString *)fromId nickName:(NSString *)nickName isFirst:(BOOL)isFirst {
    NSLog(@"LiveViewController::onRecvPushRoomFav( [接收点赞], roomId : %@, fromId : %@, nickName : %@, isFirst : %s )", roomId, fromId, nickName, isFirst ? "YES" : "NO");
...
- (void)onRecvRoomToastNotice:(NSString *)roomId fromId:(NSString *)fromId nickName:(NSString *)nickName msg:(NSString *)msg {
    NSLog(@"LiveViewController::onRecvRoomToastNotice( [接收弹幕], roomId : %@, fromId : %@, nickName : %@, msg : %@ )", roomId, fromId, nickName, msg);
...
- (void)onRecvEnterRoomNotice:(NSString *_Nonnull)roomId userId:(NSString *_Nonnull)userId nickName:(NSString *_Nonnull)nickName photoUrl:(NSString *_Nonnull)photoUrl riderId:(NSString *_Nonnull)riderId riderName:(NSString *_Nonnull)riderName riderUrl:(NSString *_Nonnull)riderUrl fansNum:(int)fansNum {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"LiveViewController::onRecvFansRoomIn( [接收观众进入直播间] )");
