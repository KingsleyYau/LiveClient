...
- (void)onSetupViewController:(PrivateViewController *)vc {
    ...
    // 消息列表界面
    self.vc.playVC.liveVC.roomStyleItem.nameColor = COLOR_WITH_16BAND_RGB(0X0CD7DE);
    ...
    [self.vc.tipView setTipWithRoomPrice:0 roomTip:NSLocalizedStringFromSelf(@"ROOM_TYPE_TIPS") creditText:nil];
    [self.vc.tipView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.equalTo(@(self.vc.roomTypeImageView.width));
    }];
