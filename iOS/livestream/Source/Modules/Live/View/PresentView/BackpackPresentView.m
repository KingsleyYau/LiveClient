...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 是否在直播间可发送列表中 是否可发送
    BOOL canSend = [[LiveGiftListManager manager] giftCanSendInLiveRoom:item];
    ...
    if (isIndexPath) {
        ...
        if (item.infoItem.loveLevel > self.loveLevel || item.infoItem.level > self.manLevel || !canSend) {
            // 发送按钮不可用
            [self sendViewNotUserEnabled];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.buttonBar.frame.size.height) {
        ...
        // 是否可发送 用户等级或亲密度不够
        int loveLevel = item.infoItem.loveLevel;
        int manLevel = item.infoItem.level;
        if (!canSend) {
