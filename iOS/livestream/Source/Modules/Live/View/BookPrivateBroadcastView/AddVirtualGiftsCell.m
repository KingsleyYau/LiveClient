...
- (void)awakeFromNib {
    ...
    self.numBtn.layer.borderColor = [UIColor colorWithHexString:@"3c3c3c"].CGColor;
    self.numBtn.layer.borderWidth = 0.5;
    
    self.numView.layer.borderColor =[UIColor colorWithHexString:@"3c3c3c"].CGColor;
...
+ (id)getUITableViewCell:(UITableView*)tableView isShowVG:(BOOL)isShowVG {
    ...
    if (isShowVG) {
        ...
        if (nil == cell){
            NSArray *nib = [[NSBundle mainBundle] loadNibNamedWithFamily:[AddVirtualGiftsCell cellIdentifier] owner:tableView options:nil];
    ...
    else
    {
        ...
        if (nil == cell) {
            ...
            cell.textLabel.text = @"Add a virtual gift";
            
        }
        
        CalvinSwitch * vgSwitch = [[CalvinSwitch alloc]initWithFrame:CGRectMake(tableView.frame.size.width - 44 -15, 15, 44, 20) onColor:[UIColor colorWithHexString:@"5d0e86"] offColor:[UIColor colorWithHexString:@"b5b5b5"] font:[UIFont systemFontOfSize:14] ballSize:16];
        vgSwitch.on = NO;
...
- (void)getVirtualGiftList:(NSArray *)array
{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        GiftItemObject * item = [array objectAtIndex:i];
        
        AllGiftItem * giftItem = [self.giftDownloadManager backGiftItemWithGiftID:item.giftId];
        
        UIButton * button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.backgroundColor = [UIColor colorWithHexString:@"dadada"];
...
- (void)loadVGNum:(NSArray *)array forGiftItem:(AllGiftItem *)giftItem
{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        UIButton * button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(0, i * 30, self.numView.width, 30);
        ...
        if (item.isDefault) {
            ...
            [self.numBtn setTitle:[NSString stringWithFormat:@"%ld",self.num] forState:UIControlStateNormal];
            
            [button setBackgroundColor:[UIColor colorWithHexString:@"f7cd3a"]];
    ...
    self.numView.contentSize = CGSizeMake(self.numView.width, 30 * array.count);
}
...
- (void)virtualGiftDid:(UIButton *)button
{
    ...
    GiftItemObject * item = [self.data objectAtIndex:button.tag - 88];
...
- (void)setPriceNum:(CGFloat)num
{
    ...
    NSString * priceStr = [NSString stringWithFormat:@"Total Price:  %0.1f crdits",num];
    ...
    [mAttStr addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"F7CD3A"] range:priceRange];
    NSRange crditsRange = [priceStr rangeOfString:@"crdits"];
    [mAttStr addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"F7CD3A"] range:crditsRange];
