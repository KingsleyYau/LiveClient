...
#ifndef LSLCLOGINFACEBOOKITEM_H_
#define LSLCLOGINFACEBOOKITEM_H_
		...
		if( root.isObject() ) {
			...
			if( root[AUTHORIZATION_PERMISSION].isObject() ) {
				...
				if( permission[AUTHORIZATION_LIVECHAT_INVITE].isInt() ) {
					livechat_invite = permission[AUTHORIZATION_LIVECHAT_INVITE].asInt() != 0;
				}
