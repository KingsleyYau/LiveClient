...
class LSLiveChatSendMsgTask : public ILSLiveChatTask
{
public:
	LSLiveChatSendMsgTask(void);
	virtual ~LSLiveChatSendMsgTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& userId, const string& message, bool illegal, int ticket, INVITE_TYPE inviteType = INVITE_TYPE_CHAT);
...
private:
	...
	string			m_message;	// 聊天消息
	bool			m_illegal;	// 是否合法
	int				m_ticket;	// 票根
    INVITE_TYPE     m_inviteType; // 消息类型
};
