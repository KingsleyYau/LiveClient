 ...
 * author: Samson.Fan
 *   date: 2015-04-01
 *   file: LSLiveChatSendVoiceTask.h
 *   desc: 发送语音Task实现类
 */
...
class LSLiveChatSendVoiceTask : public ILSLiveChatTask
{
public:
	LSLiveChatSendVoiceTask(void);
	virtual ~LSLiveChatSendVoiceTask(void);
...
public:
	// 初始化参数
	bool InitParam(const string& userId, const string& voiceId, int voiceLength, int ticket);
...
private:
	...
	string			m_voiceId;	// 语音Id
	int				m_voiceLength;	// 语音时长
	int 			m_ticket;	// 票根
};
