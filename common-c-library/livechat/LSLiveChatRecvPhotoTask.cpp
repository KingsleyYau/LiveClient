 ...
 * author: Samson.Fan
 *   date: 2015-04-11
 *   file: LSLiveChatRecvPhotoTask.cpp
 *   desc: 接收图片消息Task实现类
 */
...
LSLiveChatRecvPhotoTask::LSLiveChatRecvPhotoTask(void)
{
	m_listener = NULL;
...
// 初始化
bool LSLiveChatRecvPhotoTask::Init(ILSLiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool LSLiveChatRecvPhotoTask::Handle(const LSLiveChatTransportProtocol* tp)
{
	bool result = false;

	FileLog("LiveChatClient", "LSLiveChatRecvPhotoTask::Handle() begin, tp:%p", tp);
	...
	FileLog("LiveChatClient", "LSLiveChatRecvPhotoTask::Handle() parsing ok, tp:%p", tp);

	// 通知listener
	if (NULL != m_listener 
		&& result) 
	{
		FileLog("LiveChatClient", "LSLiveChatRecvPhotoTask::Handle() callback begin, tp:%p", tp);
		m_listener->OnRecvPhoto(m_toId, m_fromId, m_fromName, m_inviteId, m_photoId, m_sendId, m_charget, m_photoDesc, m_ticket);
		FileLog("LiveChatClient", "LSLiveChatRecvPhotoTask::Handle() callback end, tp:%p", tp);
	}
	FileLog("LiveChatClient", "LSLiveChatRecvPhotoTask::Handle() end, tp:%p", tp);
	
	return result;
}
	
// 获取待发送的数据，可先获取data长度，如：GetSendData(NULL, 0, dataLen);
bool LSLiveChatRecvPhotoTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
	// 本协议没有返回
	return result;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE LSLiveChatRecvPhotoTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}
	
// 获取命令号
int LSLiveChatRecvPhotoTask::GetCmdCode()
{
	return TCMD_RECVPHOTO;	
}

// 设置seq
void LSLiveChatRecvPhotoTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int LSLiveChatRecvPhotoTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool LSLiveChatRecvPhotoTask::IsWaitToRespond()
{
	return false;
}

// 获取处理结果
void LSLiveChatRecvPhotoTask::GetHandleResult(LSLIVECHAT_LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 未完成任务的断线通知
void LSLiveChatRecvPhotoTask::OnDisconnect()
{
	// 不用回调
}
