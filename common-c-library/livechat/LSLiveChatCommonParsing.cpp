 ...
 * author: Samson.Fan
 *   date: 2015-06-16
 *   file: LSLiveChatCommonParsing.cpp
...
#define AVATARIMG_PARAM     "avatarImg"     // 头像Url(直播使用的小头像)
...
bool ParsingUserInfoItem(amf_object_handle handle, UserInfoItem& item)
{
		...
		&& handle->type == DT_OBJECT)
	{
        ...
        // avatarImg
        amf_object_handle avatarImgObject = handle->get_child(AVATARIMG_PARAM);
        if (!avatarImgObject.isnull()
            && avatarImgObject->type == DT_STRING) {
            item.avatarImg = avatarImgObject->strValue;
        }
		...
		{
            //  服务器将deviceTypeObject->intValu = 0时默认为DEVICE_TYPE_WEB（10），其他没有的都当unknow
            if (deviceTypeObject->intValue == 0) {
                item.deviceType = DEVICE_TYPE_WEB;
                // Add by Max 2018/08/16
                item.deviceTypeOriginal = 10;
            } else {
                item.deviceType = (TDEVICE_TYPE)deviceTypeObject->intValue;
                // Add by Max 2018/08/16
                item.deviceTypeOriginal = deviceTypeObject->intValue;
            }
		}
