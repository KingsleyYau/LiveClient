 ...
 * author: Samson.Fan
 *   date: 2015-04-01
 *   file: LSLiveChatRecvWarningTask.cpp
 *   desc: 接收警告消息Task实现类
 */
...
LSLiveChatRecvWarningTask::LSLiveChatRecvWarningTask(void)
{
	m_listener = NULL;
...
// 初始化
bool LSLiveChatRecvWarningTask::Init(ILSLiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool LSLiveChatRecvWarningTask::Handle(const LSLiveChatTransportProtocol* tp)
{
	bool result = false;
...
bool LSLiveChatRecvWarningTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
	// 本协议没有返回
	return result;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE LSLiveChatRecvWarningTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}
	
// 获取命令号
int LSLiveChatRecvWarningTask::GetCmdCode()
{
	return TCMD_RECVWARNING;	
}

// 设置seq
void LSLiveChatRecvWarningTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int LSLiveChatRecvWarningTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool LSLiveChatRecvWarningTask::IsWaitToRespond()
{
	return false;
}

// 获取处理结果
void LSLiveChatRecvWarningTask::GetHandleResult(LSLIVECHAT_LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 未完成任务的断线通知
void LSLiveChatRecvWarningTask::OnDisconnect()
{
	// 不用回调
}
